...
def build_dz_profile() -> np.ndarray:
    """The FIXED z profile: uniform-fine buffer + substrate + buffer, graded.

    Rule 1's other half — every z the model uses is DERIVED from this built
    profile (``substrate_bounds`` / ``substrate_faces``), never from the
    pre-smoothing coordinates.
    """
...
def substrate_bounds(dz_profile) -> tuple[float, float]:
    """(z_sub_lo, z_sub_hi) read off the BUILT mesh, not the intended one."""
    edges = np.concatenate([[0.0], np.cumsum(dz_profile)])
    fi = np.where(np.isclose(dz_profile, dz_sub, rtol=1e-6))[0]
    f0 = int(fi[0]) + n_buf                    # skip the lower buffer cells
    return float(edges[f0]), float(edges[f0 + n_sub])


def substrate_faces(dz_profile) -> tuple[float, float]:
    """(z_gnd, z_patch) — the substrate's own two node planes.

    The ground and the patch are FOIL, so they are SHEETS on those planes —
    zero thickness, no cell of their own (#931 §1.3).  The fine band therefore
    reserves no cell for them; before the lattice ownership contract it
    reserved one each, and the ground's reserved cell sat inside the realized
    cavity carrying vacuum.
    """
    return substrate_bounds(dz_profile)


def build_simulation(dz_profile=None) -> Simulation:
    """Build the graded-mesh patch model — no solve.

    ``main()`` passes the profile it already built; called with no argument
    the builder builds the same profile itself, which is what the #737
    example-fidelity gate (tests/contracts/test_example_fidelity_contract.py)
    does to pin this tutorial's preflight/realization output without solving
    it.  Same function either way, so the audited model is the one that runs.
    """
    if dz_profile is None:
        dz_profile = build_dz_profile()
    z_sub_lo, z_sub_hi = substrate_bounds(dz_profile)
    ...
    # metal's sigma_bulk makes this a lossless PEC sheet and that `thickness`
