"""#1041 physics gate: a source in rank d's FIRST real cell must reach rank d-1.
...
GATE DERIVATION (scripts/diagnostics/issue1041_v2_step_order.py, 2026-09-15,
2 virtual CPU devices, jax 0.10.2, float32, 300 steps; max|multi - single|
divided by the single-device probe peak):

    fixture           pre-fix order    post-fix order   lane floor (interior)
...
both seam cases red at 1.859e-01 / 1.653e-01.
"""
...
def _rel_per_probe(source_x, boundary):
    ...
    ts_multi = np.asarray(
        sim_multi.run(n_steps=N_STEPS, devices=jax.devices()[:2]).time_series)
...
def test_seam_cell_source_reaches_the_neighbouring_rank(boundary):
    ...
    source's own wavefront, not an accumulated drift.
    """
    rel = _rel_per_probe(SEAM_X, boundary)
    assert rel[0] < GATE, (
        f"boundary={boundary}: source at the seam, probe 4 cells into rank 0 "
        f"deviates from the single-device lane by {rel[0]:.3e} "
        f"(gate {GATE:.0e}). The E ghost exchange must be the LAST stage of "
        "the E half-step (#1041).")
    assert rel[1] < GATE, (
        f"boundary={boundary}: source at the seam, probe inside rank 1 "
        f"deviates by {rel[1]:.3e} (gate {GATE:.0e})")


@pytest.mark.parametrize("boundary", ["pec", "cpml"])
def test_interior_source_control_sets_the_floor(boundary):
    ...
    1.629e-06 / 2.529e-05 (cpml). If this test ever approaches GATE the gate
    above has stopped separating a step-order defect from lane noise.
    """
    rel = _rel_per_probe(INTERIOR_X, boundary)
