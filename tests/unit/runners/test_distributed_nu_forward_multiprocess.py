...
def _parity():
    ...
    traces, gradients = [], []
    for eps, sigma, occupancy in (local, sharded):
        f = lambda e: _trace(sim, e, sigma, occupancy)
        traces.append(f(eps))
        gradients.append(jax.grad(lambda e: jnp.sum(f(e) ** 2))(eps))
    nx = host[0].shape[0]
    assert np.any(gradients[0]) and np.any(traces[0])
    assert gradients[1].sharding == sharded[0].sharding
    assert np.all(np.asarray(gradients[1])[nx:] == 0)
    metrics = {"trace": _within(traces[0], traces[1]),
               "gradient": _within(gradients[0], np.asarray(gradients[1])[:nx])}
...
def _worker(address, rank, output, mode):
    ...
    if mode == "refuse":
        ...
        def local(index):
            ...
            return (1.2 + (indices % 71) * .05).astype(np.float32).reshape(
                hi - lo, *shape[1:])

        eps = jax.make_array_from_callback(shape, sharding, local)
        f = lambda e: _trace(sim, e)
        trace = f(eps)
        grad = jax.grad(lambda e: jnp.sum(f(e) ** 2))(eps)
        assert grad.sharding == eps.sharding
        ...
        for shard in grad.addressable_shards:
            np.save(Path(output) / f"grad-{shard.device.id}.npy", np.asarray(shard.data))
        print(f"rank={rank} plain+grad ok; trace replicated; gradient={grad.sharding}", flush=True)
    if address != "reference":
...
def test_two_process_plain_and_grad(two_process_results):
    ...
    actual = np.concatenate([np.load(p) for p in sorted(
        output.glob("grad-*.npy"), key=lambda p: int(p.stem.split("-")[1]))])
    expected = np.concatenate([np.load(p) for p in sorted(reference.glob("grad-*.npy"))])
    assert np.any(expected), "non-vacuous multi-process gradient"
    print("RESULT gradient", _within(expected, actual))
