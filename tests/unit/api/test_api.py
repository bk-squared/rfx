...
@pytest.mark.skip(
    reason=(
        ...
        "reference-run design gap: the shared VACUUM reference (no interior PEC) "
        "radiates into free space, so the guided incident power P_inc is "
        "mis-normalized and |S| inflates (normalize=True max|S|~230; "
        "normalize='flux' max|S|~9.8, |S11|~1.9). Verified 2026-07-05: supplying "
        "PEC-folded per-port straight-guide references is "
        "NECESSARY but NOT SUFFICIENT. The public plumbing now exists via "
        "compute_waveguide_s_matrix(port_reference_sims=[...]) (each a matched "
        "straight guide for a driven port), but the COMPACT geometry here stays "
        "non-physical — a physical junction S-matrix additionally requires the "
        "far-port discipline (probe clearance >= 5 evanescent decay lengths, "
        "CPML >= ~0.5 guide wavelengths, converged mesh); see the "
        "compute_waveguide_s_matrix docstring and "
        "tests/unit/sparams/test_waveguide_port_reference_sims.py. Concretely it fixes |S11| "
        "(1.9->0.49, physical) and "
        "halves the blow-up (max|S| 9.8->3.9), but does NOT reach passivity "
        "(residual max|S|~3.9, per-drive energy sum 2-6x). R2-STOP: one clean "
        "attempt, not chased. NOTE: this test's own gates check only cross-"
        "coupling magnitude (>0.15) + reciprocity (<0.2) with NO passivity check, "
