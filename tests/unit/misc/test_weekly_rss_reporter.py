...
def test_malformed_threshold_env_var_does_not_break_collection():
    ...
    result = subprocess.run(
        [sys.executable, "-m", "pytest", "--collect-only", "-q", "tests/unit/misc/test_weekly_rss_reporter.py"],
        cwd=str(root),
        env=env,
        capture_output=True,
        text=True,
...
def _run_sigkill_helper_subprocess(extra_pytest_args):
    ...
    return subprocess.run(
        [
            ...
            "tests/unit/misc/test_weekly_rss_reporter.py::test_rss_live_sampler_helper_allocates_and_self_sigkills",
        ],
        cwd=str(root),
        env=env,
        capture_output=True,
