"""Self-checks of the slab-guide dispersion helper (``tests/_slab_te_dispersion.py``).

These gate the INSTRUMENT, not the physics. The closed-form TE0 effective index
of a symmetric dielectric slab is checked against an independent
finite-difference eigensolve that shares no line of code with it, and the
two-wave estimator is checked against synthetic lines whose beta is known
exactly.

The helper's live consumer is
``tests/unit/boundaries/test_boundary_touching_dielectric_pad_continuation.py``,
which measures the round trip on a guide that runs into the absorber. These
tests came from the former cv03 straight-guide cross-validation case, removed
2026-09-21; the four tests that re-derived that case's committed artifact went
with it.
"""
import math
import sys
...
from tests._slab_te_dispersion import (
