...
def violations(sim, grid, arrays, poles, nodes):
    ...
    for axis in range(3):
        for side in (0, 1):
            pad = grid.face_pads[2*axis+side]
            if not pad:
                continue
            ...
            for shape, name in entries:
                ...
                if reason is None:
                    continue
                # Incident edges and tensor transition cells occupy the cell
                # touching either side of each declared transverse boundary.
                window = np.ones(shape2, bool)
                for dim, a in enumerate(others):
                    line = np.asarray(nodes[a])
                    hit = (line[:-1] <= hi[a]) & (line[1:] >= lo[a])
                    window &= hit[:, None] if dim == 0 else hit[None, :]
                exempt |= window
                exceptions.append((axis, side, reason, int(window.sum())))
            for name, raw in arrays.items():
                ...
                for layer in layers:
                    ...
                    count += int(np.count_nonzero(diff & ~exempt))
                if count:
