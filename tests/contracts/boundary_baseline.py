        ...
        with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
            try:
                ...
                assert fields.shape[-3:] == grid.shape, (
                    ...
                    "the leader must resolve the full-grid versus ghosted-slab selector before regeneration")
                faces = classify(fields, grid, psi, reference)
                row.update(status="MEASURED", declared=declared, shape=grid.shape, faces=faces,
                           scan_records=[dict(rank=r["rank"], shape=r["fields"].shape) for r in records],
...
def main(argv=None):
    ...
    for row in cells:
        old = old_lookup[row["case"], row["entry"]]
        before = {(d["face"], d["code"]) for d in old.get("departures", [])}
        after = {(d["face"], d["code"]) for d in row.get("departures", [])}
        if old["status"] != row["status"] or before != after:
            raise AssertionError(f"STOP: classification changed in {row['case']}/{row['entry']}: {before} -> {after}")
    ...
    print(f"{len(cells)} cells; {sum(r['status'] == 'MEASURED' for r in cells)} measured; "
          ...
          f"{baseline['cpu_seconds_cells']:.6f} cell CPU s; base {base}; no class changes")
