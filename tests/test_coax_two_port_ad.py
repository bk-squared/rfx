...
floor (1e4, matching ``tests/test_msl_ad_fd_converged.py``'s own value) --
...
``tests/test_msl_ad_fd_converged.py::test_comparator_floor_rejects_the_f32_
...
# matter how exact the solver is -- see tests/test_msl_ad_fd_converged.py's
...
def _fd_ulp_span(f_plus: float, f_minus: float, dtype) -> float:
    ...
    Mirrors ``tests/test_msl_ad_fd_converged.py::_fd_ulp_span`` exactly (not
...
def test_compute_coaxial_two_port_ad_grad_finite_and_fd_consistent():
    ...
    ``tests/test_msl_ad_fd_converged.py`` established after issue #527). See
...
def test_coax_two_port_eps_scale_unity_matches_concrete_path():
    ...
    mirrors ``tests/test_coax_end_to_end_ad.py::
