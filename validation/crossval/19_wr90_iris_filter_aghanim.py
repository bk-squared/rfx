...
# shipped green as long as the bridged midpoint stayed inside the f0 gate.
# A second hole bin is a regression finding requiring root-cause, not noise.
...
def iris_thickness_zero_count_sweep(geo, lo_cells=8.0, hi_cells=8.5,
    ...
    for i in range(n):
        t = lo_cells + i * step_cells
        g = dict(geo)
        g["thicknesses"] = [t * dx] * len(geo["thicknesses"])
        ba = band_analysis(oracle_curve(g, n_a=n_a))
        rows.append({"t_elec_cells": round(t, 2),
                     "zeros": len(ba["zeros"]),
                     "f0_hz": round(ba["f0"]),
                     "bw_hz": round(ba["bw"]),
                     "span_holes": ba["span_holes"]})
    return rows
...
def main(argv):
    ...
    contig_ok = meas["span_holes"] <= MAX_SPAN_HOLES_GATED
    ...
    if write_fixture:
        ...
        tz = iris_thickness_zero_count_sweep(geo_g_e)
        tz_ok = all(r["zeros"] == len(ras["zeros"]) for r in tz)
        print(f"  t_elec {tz[0]['t_elec_cells']:.2f}-{tz[-1]['t_elec_cells']:.2f} "
              f"cells ({len(tz)} points): zero count "
              + (f"INVARIANT at {len(ras['zeros'])}" if tz_ok else
                 f"NOT INVARIANT: {[r['zeros'] for r in tz]}")
              + f"; bw moves {(tz[0]['bw_hz'] - tz[-1]['bw_hz']) / 1e6:+.1f} MHz "
              f"across the band")
        ok &= tz_ok
        ...
        payload = {
            ...
            "claim_scope": "A published 4th-order WR-90 inductive-iris bandpass filter (Aghanim et "
                           "al., E3S Web of Conferences 351, 01059 (2022), CC BY 4.0, Table 6 "
                           "optimized: five irises t = 2.00 mm, apertures "
                           "10.27/6.65/6.18/6.65/10.27 mm, cavities 14.29/15.73/15.73/14.29 mm) "
                           "built at dx = a/90 and compared against a TEn0 mode-matching cascade "
                           "oracle over 10.40-11.70 GHz on 131 points at 10 MHz. Stage S3 of the "
                           ...
                           "joint review showed f0 is computed from the OUTERMOST -10 dB crossings, "
                           ...
                           "standoff 3.05/7.62/15.24 mm, absorber depth 0.75/1.00/1.25 lambda_g. "
                           "The interior samples are the point rather than decoration: a "
                           "one-alternative-per-axis envelope cannot detect NON-MONOTONIC "
                           "sensitivity, which is exactly the failure of PR #475, where three "
                           "sampled clearances passed while 9 of 13 exceeded the gate and the "
                           "passing three were the sampled ones. Every population member carries "
                           "its own committed |S11| trace, so each residual is recomputable rather "
                           "than a free-floating scalar whose integrity is borrowed from asserts "
                           ...
                           "merged case 18's gate is; what gives the measured agreement meaning is "
                           "not the gate but the comparison of that agreement against an external "
                           "scale, namely the reference's own 21.9 MHz f0 spread between two "
                           "independent commercial codes. That tightness is the substance of the "
                           "result: the residual is a reproducible systematic difference rather "
                           "than a setup artifact, and at the measured cavity sensitivity of -105 "
                           "MHz/cell it corresponds to about 0.12 cell of cavity length. The "
                           "num_periods = 200 run is EXCLUDED from the envelope rather than folded "
                           ...
                           "meaningful, depths are not values); passband contiguity beyond the "
                           "span_holes <= 1 lock (hole depth, hole position, longest contiguous "
                           ...
                           "the minimum over samples already filtered to >= 10 dB and therefore "
                           "could not report a violation at all. The coarse a/60 rung has no "
                           "meaningful passband: 16 of 24 bins in its nominal span are above -10 dB "
                           "with an interior trough at 2.73 dB, i.e. two separated resonances. "
                           ...
                           "measurement. The repo's preferred ENERGY-BASED ring-down witness "
                           "(terminal energy in dB below the post-source peak, rule < -40 dB) is "
                           "NOT AVAILABLE on this path: it is implemented for the lumped/MSL "
                           "S-matrix extractor, but compute_waveguide_s_matrix returns no "
                           "settling_db, and the null fields are committed in every row rather than "
                           "papered over (filed as an rfx capability gap). The ENFORCED settling "
                           "criterion is therefore the independent axis: the num_periods scan "
                           "400/600/800 holds f0 and BW to under 0.1 MHz (the gate is the 400 -> "
                           "800 doubling within one 10 MHz bin), the gated run is passivity-clean "
                           ...
                           "absorber-depth scans each hold the edges to one 10 MHz bin across their "
                           "interior and outer samples (standoff 3.05 -> 7.62 -> 15.24 mm: 0.0/0.1 "
                           "MHz; absorber 0.75 -> 1.00 -> 1.25 lambda_g: 0.0/0.1 MHz). Absorber "
                           "depth is scanned because in S1 it was the envelope-limiting term at 0.5 "
                           "lambda_g; here 0.75 lambda_g is measurably sufficient, which is a "
                           "negative result worth recording rather than a rule inherited. Extractor "
                           ...
                           "Its own witnesses are unitarity 2.2e-15, reciprocity and mirror "
                           "symmetry exact, and an L -> 0 collapse closing two thin irises onto one "
                           "thick one; its N=1 centred limit reproduces the merged case-18 oracle "
                           "to 1.05e-04 in an independent odd-mode formulation, and THAT object is "
                           "what PR #480 confirmed against a formulation-independent 2-D H-plane "
                           "FDFD at 5.8e-4 - a comparison the frozen gate test now EXECUTES rather "
                           "than asserting in prose. Those witnesses have known limits, stated "
                           "because an earlier revision leaned on them too hard: unitarity "
                           "constrains only the propagating sub-block, mirror symmetry holds by "
                           "construction for a symmetric geometry, and injected overlap-integral "
                           "errors leave the reduction and collapse axes silent because they share "
                           "the overlap routine, so the gate test's re-typed cascade agreeing to "
                           "0.0e+00 is a REGRESSION LOCK and not a second opinion. An independent "
                           "review then closed the real gaps. What is COMMITTED: the overlap "
                           "integral is validated against direct numerical quadrature IN CI (216 "
                           "combinations across three apertures including the as-realized 6.096 mm "
                           "-- where n*pi/a equals m*pi/d exactly for six mode pairs -- centred and "
                           "0.19 mm off-centre, worst deviation bounded at 1e-12 with the "
                           "small-denominator guard exercised), and the oracle's truncation is "
                           "witnessed on the GATED observable at generation time: f0 by bisection "
                           "moves 0.33 MHz for n_a 90 -> 130 and 1.85 MHz for an "
                           "aperture-mode-count doubling -- the aperture axis is the sensitive one, "
                           "and both are an order under the 19 MHz gate. Session measurements "
                           "recorded in the research notes but NOT in this record put the gauge "
                           "invariance of the sqrt(Y) normalisation at machine precision, "
                           "truncation saturation near 1 MHz in bandwidth, and inter-cavity "
                           "evanescent transport near 1.6 MHz; they are corroborating colour rather "
                           "than load-bearing, because the formulation-level check below subsumes "
                           "their role. THE FORMULATION-LEVEL CHECK IS NOW DONE, and it lands the "
                           "residual on the rfx side. A 2-D H-plane FDFD -- scalar Helmholtz on a "
                           ...
                           "was run on the same electrical geometry, grid-exact at every refinement "
                           "level. It is FIRST-order convergent, so no single level is meaningful; "
                           "the record carries THREE levels (r = 2, 3, 4, whose bandwidth "
                           "deviations from the extrapolate shrink as 1/r: measured ratios 1.55 and "
                           "1.33 against the first-order 1.50 and 1.33) and BOTH Richardson "
                           "estimates, which agree to 0.37 MHz in centre frequency and 0.36 MHz in "
                           "bandwidth -- the two-estimate consistency protocol the porting handoff "
                           "mandates before either estimate is trusted. The finer pair gives f0 = "
                           "10.95742 GHz and BW = 351.42 MHz against the cascade's 10.95851 and "
                           "350.43: agreement to 1.09 MHz in centre frequency and 0.98 MHz in "
                           "bandwidth between two formulations that share nothing but their "
                           "numerical libraries, with the extrapolation's own order-assumption "
                           "uncertainty, not the two-estimate consistency, bounding how much of "
                           "that gap the FDFD itself owns: the consistency (0.37/0.36 MHz) is "
                           "agreement between two fits of the SAME first-order model, not an error "
                           "bound -- fitting the convergence order to the three committed levels "
                           "edge-by-edge gives p = 1.26 (lower edge) / 1.24 (upper edge) rather "
                           "than the assumed 1, and re-extrapolating the finer pair at the fitted "
                           ...
                           "in the comparator itself was found by an independent port review and "
                           "fixed before this record was generated: its aperture mask realized "
                           "every aperture two fine cells wide of the stated convention, a "
                           "first-order bias that Richardson cancelled -- making the extrapolated "
                           "numbers right for the wrong per-level geometry -- and that produced a "
                           "spurious FOURTH reflection zero at the coarser levels. With the mask "
                           "exact, every level shows THREE zeros, matching the cascade and rfx. "
                           "Independently, the cascade's zero count was checked against its own "
                           ...
                           "EXPERIMENTAL; this measures one published design on one mesh with one "
                           "gated observable, and certifies neither arbitrary filters nor the a/60 "
                           "rung. Says nothing about phase, group delay, loss, higher-order-mode "
                           "ports, or fabrication tolerance. The reference is an ANCHOR, not a "
                           "solver run: the CST and HFSS scalars are digitized from the paper's "
                           "Fig. 5, no external solver is invoked here, and the case does not "
                           "compare rfx against either commercial code on any geometry - so the "
                           "12.1 MHz f0 residual against this case's own analytic oracle must not "
                           "be read as an accuracy claim relative to CST or HFSS. Reported for "
                           "context and not as a yardstick beaten: the oracle on nominal dimensions "
                           "sits -6.2 MHz from CST and -28.2 MHz from HFSS in f0, +14.7 MHz from "
                           "both in bandwidth (against a published inter-solver bandwidth spread of "
                           "only 0.4 MHz), -0.4 and -1.1 dB in worst return loss, and up to 25.4 "
                           "and 61.9 MHz in individual reflection-zero frequencies. The bandwidth "
                           "and zero-frequency misses are larger than the f0 miss and are stated "
                           "here because quoting only f0 would be selective. The built structure is "
                           "a SNAPPED Aghanim filter: its centre frequency is within the "
                           "reference's own solver scatter of CST (though not of HFSS), one of the "
                           ...
                           "every snap figure quoted here was MEASURED on the as-snapped geometry "
                           "and none may be re-derived by multiplying a sensitivity coefficient by "
                           "a half cell.",
            "config": {
