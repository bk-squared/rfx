...
# Falsifier hook (#812 round 2): the committed ring-down fixture
# tests/fixtures/patch_mode_identification/cv05_ringdown_spectra.json is
# regenerated by scripts/diagnostics/build_cv05_ringdown_spectra.py, which
# runs THIS script with the resonant length mis-realized on purpose. Unset
# in every normal run; the declared length above is what the case solves.
_L_override_mm = os.environ.get("RFX_CV05_PATCH_L_MM", "")
if _L_override_mm:
    L = float(_L_override_mm) * 1e-3
    print(f"[falsifier hook] RFX_CV05_PATCH_L_MM={_L_override_mm}: patch length set to {L*1e3:.4f} mm")
...
if not np.isnan(f_res_harminv):
    ...
    print(f"  Harminv error:      {harminv_err_pct:.2f} %  (REPORTED as accuracy; "
          f"identification bounds it to [-11.11 %, +12.50 %])")
else:
