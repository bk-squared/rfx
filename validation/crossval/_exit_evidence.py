...
TESTING IT ON THE REAL CASES. A passing run never exercises the amendment, so
``RFX_EXIT_EVIDENCE_SELFTEST`` makes a migrated case leave by a different door
immediately after its record is written -- see ``_selftest_late_exit`` for the
spelling and for the two guards that keep the knob away from committed
evidence. ``tests/crossval/test_crossval_exit_code_is_the_process_exit_code.py``
drives cv01 and cv02 through it in subprocesses and compares the persisted code
with the subprocess's own return code.
"""
...
#: Env var that makes a migrated case leave by a DIFFERENT door than the one
#: its verdict stage declared, so the mechanism can be tested on the real
#: scripts rather than on a fixture that imitates them. See
#: ``_selftest_late_exit``.
...
def _selftest_late_exit(record_path: str, declared: int) -> None:
    ...
    ONE record it names::
        ...
        RFX_EXIT_EVIDENCE_SELFTEST="raise|/abs/path/to/record.json"
        RFX_EXIT_EVIDENCE_SELFTEST="exit:3|/abs/path/to/record.json"
