...
| `crossval/07_sheen_lpf.py` | Sheen 1990 stepped-impedance microstrip low-pass filter stopband | Characterization study, not an accuracy comparison. A committed Palace FEM referee shows the stopband is a double transmission zero (~7.0 and ~8.0 GHz) that OpenEMS reproduces to about 0.7% while rfx at dx=200 µm distorts it, so the referee sides with OpenEMS and the single "first null" distance of 9.6% is a comparator artifact, reported but not gated. The checks lock the OpenEMS reference's own soundness, the shape of the comparison, the committed null frequencies and referee verdict, and the footprint of a known extraction defect. That defect is the reason to read this case narrowly: the committed rfx result is not passive — 58 of its 120 bins exceed the documented column-power limit of 1.10, 29 of them inside the 5–15 GHz band the study reads from, worst at 17.05 GHz where column power reaches 1794.5 and `Re(Z₀)` goes negative. No rfx magnitude from this study may be quoted as a result. |
