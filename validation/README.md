...
| `crossval/16_pec_sphere_mie_ka_sweep.py` | PEC-sphere monostatic RCS vs exact Mie, ka 0.5–4.0 | Two-tier: coarse ka ≤ 1.25 and a fine rung at ka {2.0, 4.0} are gated against a twice-independently re-derived Mie series (measured-envelope × 1.5); coarse ka ≥ 1.5 and fine ka = 3.0 are reported but never gated — near the deep Mie nulls the monostatic value is domain-size unconverged (up to 6.3–8.3 dB swing, minima positions move), with the attribution probes (truncation, effective radius, incident leakage: all falsified) frozen in the fixture. Says nothing about bistatic bins, dielectric spheres, or oblique incidence. |
