...
R(f)/T(f) of periodic unit cells -- uniform slabs today (Tier-A, gated
below); patterned FSS (e.g. a Munk-class dipole array, per #491 step 2) once
a grcwa `Add_LayerGrid` case is added here -- as an independent cross-check
for rfx's future calibrated Floquet S-parameters.
...
verified by cloning that tag and running its pytest suite green against the
`pip install grcwa` package used here) -- a single patterned layer
(100x100-pixel circular hole, radius 0.4 of the unit cell, eps=12 fill /
eps=1 background), nG=101, oblique incidence theta=pi/18, phi=pi/9,
compared against the two DOCUMENTED expected transmittances the grcwa
authors themselves cross-validated against the independent RCWA
implementation S4 (https://github.com/victorliu/S4): T_p = 0.85249901083265
and T_s = 0.83900479939861, both to grcwa's own relative tolerance
...
Step 2 -- TIER-A CROSS-ANCHOR (instrument calibration on a case with
independent analytic truth). `run_tier_a_slab_check()` runs a uniform
eps_r=4.0, d=10 mm dielectric slab -- the SAME fixture rfx's own cv04
(`validation/crossval/04_multilayer_fresnel.py`) validates its TFSF/Fresnel
extraction against -- through grcwa at broadside (theta=phi=0) and zeroth
order (nG=2, grcwa's minimum truncation that resolves to a single -- the
zeroth -- diffraction order; see the nG gotcha note below) and gates the
result against the SAME closed-form Airy/Fresnel transfer-matrix formula
cv04 uses (`fresnel_slab_RT`, ported here with attribution, unmodified).
This proves the referee's power-normalization (`RT_Solve(normalize=1)`) and
unit convention (grcwa's natural units, c=1; this script nondimensionalizes
with L0 = 1 mm) are correct on a case with independent ground truth,
INDEPENDENT of anything rfx computes -- it calibrates the referee, not rfx.
Measured result: max|R_rcwa - R_analytic| and max|T_rcwa - T_analytic| are
both at floating-point noise level (~1e-15) over a 1-20 GHz sweep -- see the
committed artifact/log for the full per-frequency table.
...
here): (a) energy conservation R+T=1 is checked at every Tier-A frequency
(lossless dielectric, no loss channel available to absorb a discrepancy);
(b) the reproduce-gate's tolerance is grcwa's own committed `tolS4`, not a
loosened one; (c) the nG truncation was probed against
...
"zeroth order" means operationally in this script. This is a probed gotcha
an external tool does not warn about on its own, not an rfx preflight check.
...
def fresnel_slab_RT(freqs_hz, eps_r, d_m):
    ...
    TFSF/Fresnel extraction against. Reused here as the Tier-A cross-anchor
    for the RCWA referee itself, not for any rfx comparison.
    """
...
def rcwa_slab_RT(freq_hz, eps_slab, d_slab_m, L0_m=1.0e-3, nG=2):
    """Zeroth-order, broadside R/T of a uniform dielectric slab via grcwa.
...
def run_tier_a_slab_check(freqs_hz, eps_slab=4.0, d_slab_m=10.0e-3):
    """Gate grcwa's broadside zeroth-order slab R/T against the closed-form oracle.
    ...
    referee's own calibration case.
    """
...
def main() -> int:
    ...
    print("\n--- Step 2: Tier-A cross-anchor (uniform slab vs closed-form Fresnel) ---")
    freqs_hz = np.linspace(1.0e9, 20.0e9, 20).tolist()
    tier_a = run_tier_a_slab_check(freqs_hz)
    ...
    print(f"  max|R_rcwa-R_analytic| = {tier_a['max_dev_R']:.3e}")
    print(f"  max|T_rcwa-T_analytic| = {tier_a['max_dev_T']:.3e}")
    print(
        f"  max energy-conservation deviation |R+T-1| = "
        f"{tier_a['max_energy_conservation_dev']:.3e}"
    )
    print(f"  Tier-A PASSED: {tier_a['passed']}")

    overall_passed = bool(reproduce["passed"] and tier_a["passed"])
