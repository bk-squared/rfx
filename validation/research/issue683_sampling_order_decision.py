...
HISTORICAL NOTE (2026-08-29, follow-up branch agent/issue-683-uniform-flip):
after the verdict below, the uniform lane's wire-port sampling flipped to
POST-injection in production. On flipped code the "PRE" arm label no longer
describes the uniform lane — both arms sample post-injection, both pass the
circuit law, and this script's F2 clause correctly reports INCONCLUSIVE
(the arms are no longer a pre/post toggle; G2 fails by design). The
flip's acceptance gate is validation/research/issue683_flip_acceptance.py,
which re-applies the section-6 rule to the uniform arm alone.
