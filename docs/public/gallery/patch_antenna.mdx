...
The construction below mirrors `validation/crossval/05_patch_antenna.py`, on a
...
resonance; the matched dip and the ring-down coincide to
...
The quantity under test is the rfx resonance frequency, checked against the
first-order analytic transmission-line (Balanis) estimate. The rfx numbers below
are read from the simulation itself, not from an independent solver.
