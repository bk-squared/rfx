...
3D scheme since #338) removes the explicit CFL stability restriction for the
homogeneous, lossless split with compatible domain boundaries.
`adi_cfl_factor` scales the grid's Yee timestep; accuracy still depends on it.
This guarantee does **not** extend to the current interior PEC projection.

**Interior PEC is refused on both 3D and 2D TMz ADI, at every factor.**
Preflight reports the error `adi_interior_pec_unsupported`; `run()`, `forward()`,
and the low-level ADI step/run functions enforce it even when preflight is
skipped. Sheets, wires, and volumes must be retained: use `solver="yee"`.
Domain-boundary PEC without interior conductors remains supported.

The shipped factor 5 produced growing and non-finite sheet traces on both
lanes. Factor 1 also grew over longer runs (3D sheet peak 4.33e6; TMz sheet
peak 1.77e19 at 4,000 steps). Lowering or clamping the factor is therefore not
a supported remedy. A supplied internal mask is rejected structurally, even
when all entries are false, so the guard also works under JAX tracing; use
`None` for an absent mask. Differentiable PEC occupancy overrides have no ADI
carrier and are explicitly refused too.
...
# Closed vacuum PEC cavity, the same box the runnable demo below resonates.
a, b, d = 24.0e-3, 12.0e-3, 36.0e-3
dx = 1.5e-3
...
- **3D accuracy is validated against an analytic oracle.** A lossless 3D PEC
  ...
  `adi_cfl_factor = 2` (`tests/unit/misc/test_review_tier1_validation_battery.py`,
  measured 1.2%). The 2D TMz path (`mode="2d_tmz"`) holds a 2% resonance gate at
  5× CFL (`tests/unit/runners/test_adi.py::test_adi_cavity_resonance`). These
  cavities contain no interior conductor; they do not validate internal PEC
  or arbitrary material discontinuities.
- **The stiff-mesh throughput advantage is not yet demonstrated.** The larger
  timestep is permitted by the homogeneous ADI stability analysis, but a
  measured end-to-end speed-and-accuracy result on a stiff production
...
2, ADI at factor 5 — and prints the frequency error each way.
