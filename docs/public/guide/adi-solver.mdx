...
3D scheme since #338) is **unconditionally stable**: it stays bounded at any
timestep.
...
- **3D accuracy is validated against an analytic oracle.** A lossless 3D PEC
  ...
  5× CFL (`tests/unit/runners/test_adi.py::test_adi_cavity_resonance`) and is unconditionally
