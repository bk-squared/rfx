...
use `normalize=False` (default) or `normalize="flux"`. If a passive
`normalize=False` result rises above the documented single-run over-unity
envelope, rfx emits a soft advisory rather than silently treating it as ordinary
evidence; use the guide-recommended flux normalization or a reference check
before promoting that number.
...
| `add_lumped_rlc()` | Lumped circuit/load element | no direct S-parameter calculator; can be a differentiable scalar load via `forward(rlc_values_override=...)` on the uniform single-device lane |
