...
Setup used by the source and lumped-port snippets below:
...
sim.add_source(
    ...
    amplitude_kind="current",         # or "field"; see below
)
```
...
(issue #571). With `amplitude_kind='current'` the amplitude is a current
`I(t)` in amperes, injected as `E += Cb·I/dV` on every path and boundary
(`Cb = (dt/ε)/(1 + σ·dt/(2ε))` at the source cell, `dV` its cell volume), so
the injected power does not change when the mesh is refined — Meep's
convention. With `amplitude_kind='field'` the amplitude is a raw E-field
increment per step, `E += w(t)`, again the same on every path and boundary.
Omitting it
...
`amplitude_kind` becomes required in 1.9, and the default becomes
`'current'` in 2.0.
...
```python
# A wire port cannot share a Simulation with the lumped port above, so this
# section starts a fresh one.
sim = Simulation(
    freq_max=6e9,
    domain=(0.06, 0.06, 0.012),
    dx=1e-3,
    boundary="cpml",
    cpml_layers=6,
)
...
R = 75.0   # the overridden resistance; pass a tracer here under jax.grad
...
```python
# A microstrip port needs its own board, and cannot share a Simulation with
# the lumped/wire ports above. Ground, substrate, and trace are explicit.
sim = Simulation(
    freq_max=6e9,
...
sim.add_material("substrate", eps_r=3.2, sigma=0.01)
# 2-cell PEC volumes, not foil: each realizes walls at both drawn faces, so the
# electrical substrate is the declared 1.00 mm. Foil would be a sheet instead.
sim.add(Box((0.75e-3, 0.75e-3, 0.75e-3), (19.25e-3, 9.25e-3, 1.25e-3)), material="pec")
sim.add(Box((0.75e-3, 0.75e-3, 1.25e-3), (19.25e-3, 9.25e-3, 2.25e-3)), material="substrate")
sim.add(Box((0.75e-3, 4.50e-3, 2.25e-3), (19.25e-3, 5.50e-3, 2.75e-3)), material="pec")

sim.add_msl_port(
    position=(0.004, 0.005, 1.25e-3),
    width=1.0e-3,
    height=1.0e-3,
    direction="+x",
    impedance=50.0,
...
This teaching board is coarse — 0.25 mm cells across a 1 mm substrate — so the
run prints the extractor's advisory that the reported Z0 deviates from the
analytic Hammerstad–Jensen value by more than 10 %. Z0 rides on the N-probe fit,
which carries Yee-staircase bias at this cell size, and the rasterized board is
not exactly the declared one (issue #752). The advisory also reports this port's
probe clearance, and on this board it is satisfied — so standing-wave content at
the probes is not the cause here, and the remedy is a finer mesh, not a longer
feed. The fitted Z0 and beta do not enter S11/S21, which use the measured V/I
and the analytic Z0 anchor; that is a dataflow fact and not a certificate for
those inputs. Refine the mesh before quoting Z0.
...
```python
# A waveguide port cannot share a Simulation with lumped ports, so this
# section starts a fresh guide.
sim = Simulation(
    freq_max=11e9,
...
```python
# run() supports one waveguide port, so drive a single-port guide here.
sim = Simulation(
    freq_max=11e9,
    domain=(0.10, 0.023, 0.010),
    dx=1e-3,
...
```python
# TFSF cannot share a Simulation with lumped or waveguide ports.
sim = Simulation(
    freq_max=8e9,
...
| `add_msl_port()` | Full-strip microstrip S-matrix | `compute_msl_s_matrix()`; Laplace/quasi-TEM accepts `+x`, `-x`, `+y`, and `-y`; the x-only eigenmode code path remains outside the public support envelope |
