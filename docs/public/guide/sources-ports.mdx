...
Its current public support is limited / under active validation. Use it for the documented uniform-Yee lumped-port workflows and check examples before broadening the claim.
...
Current support is specialized and still being broadened: the thru-line and notch examples
...
into the same modal basis. Current support is recommended within the documented
...
| `add_coaxial_port()` | Coaxial transmission-line reflection | `compute_coaxial_line_reflection()` inside the documented one-port line envelope; older `compute_coaxial_s_matrix()` is deprecated/experimental |
