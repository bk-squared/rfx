...
your observable needs. Hold geometry, source, probe and run duration fixed
across the sweep — only `dx` changes. The example selects the lowest mode in a
fixed band, so the same mode is tracked at every resolution, and it handles an
empty `find_resonances(...)` result explicitly rather than indexing into it.
