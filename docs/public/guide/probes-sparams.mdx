...
`flux_spectrum(...)` checks eager float32 results that are exactly zero at every
frequency even though their DFT accumulators are nonzero. If a float64 recompute
of the same Poynting sum is nonzero, rfx warns that the per-cell `E × H*`
products underflowed and were flushed to zero. The returned float32 array is not
rewritten. Enable x64 for that computation, increase the source amplitude, or
recompute from the healthy accumulators in float64; do not interpret the exact
zeros as a physical transmission null.
...
The eager S-parameter calculators also run their matching finiteness and
passivity checks before returning. Warnings identify the worst affected bin but
leave every computed value intact. A passive result with a material `|S| > 1`
or column power above its calculator envelope is an extraction, settling,
normalization, or reference-plane problem until a cross-check proves otherwise.
...
reliable = msl.reliable  # (n_ports, n_freqs) bool, or None while tracing
```
...
A deep standing-wave node can collapse both voltage and current at an MSL port
plane, making the single-run wave split ill-conditioned. When eager phasors are
available, `MSLSMatrixResult.reliable[p, k]` is `False` if both `|V|` and `|I|`
at port `p`, frequency bin `k` fall below 10% of their per-port band medians.
The corresponding `S` values remain present but should not be plotted, fitted,
or optimized as physical data without another reference plane or independent
calculation:
