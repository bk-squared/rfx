...
setup. Setup used by the snippet below:

```python
import jax.numpy as jnp
from rfx import Box, GaussianPulse, Simulation

# A closed PEC box keeps every grid index a physical cell, so the check below
# never lands in an absorber.
sim = Simulation(
    freq_max=5e9,
...
# The design variable is the permittivity field; its shape is the grid shape.
...
[Inverse Design](/rfx/guide/inverse-design/) covers the rfx optimization API and
objective families.
