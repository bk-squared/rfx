...
arrays; it is a `rfx.batch` helper rather than part of the documented
star-import API surface. The `output_fn` should return a one-dimensional
...
# The factory and the sweep from sections 2 and 1 are reused unchanged.
...
summary = summarize_batch_manifest("runs/inclusion-sweep/manifest.json")
