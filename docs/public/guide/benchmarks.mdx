...
| Diagnostic only | `05_patch_antenna` | 2.4 GHz rectangular patch resonance on FR4 (coarse-mesh integration check) | **OpenEMS** "Simple Patch Antenna" tutorial and **Balanis** transmission-line estimate, both reported not gated | Reports a coarse-mesh (dx=1 mm) resonance and exercises the probe-fed patch / finite-GP / CPML / non-uniform-z / openEMS-lumped-port path end to end. It does not gate accuracy; patch-accuracy evidence lives in the committed tests `tests/test_patch_edgefed_resonance_harminv.py`, `tests/test_patch_edgefed_s11_passivity.py`, `tests/test_patch_cavity_eps_oracle.py`, and `tests/test_patch_canonical_farfield_e4.py` (committed thirds-rule OpenEMS far-field reference, envelope-locked). | `python validation/crossval/05_patch_antenna.py`<br />`pytest -q -m slow tests/test_patch_canonical_farfield_e4.py` |
