...
- Harminv, RCS, and inverse-design docs now pin recent footguns: lossless closed-cavity Q is a window-length artifact, RCS validation is monostatic-only unless additional evidence is supplied, and a finite-difference gradient check does not prove an empty observation window is physically meaningful.
