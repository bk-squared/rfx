...
| rectangular waveguide S-matrices; coaxial transmission-line reflection | port-family evidence envelopes; claims-bearing only inside documented geometry/frequency/mesh limits |
| distributed execution, Floquet/Bloch, SBP-SAT subgridding, generalized planar/advanced ports, inverse-design extensions | experimental or under active validation |
...
Lead with the recommended default lane. Mention promoted port-family envelopes only with their current limitations, and mention experimental lanes only with the examples that apply to the user’s workflow.
