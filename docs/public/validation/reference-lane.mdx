...
This page is the reference lane (its URL slug): the configuration to start from
and the calculations whose limits are narrower than it.
...
correctness, or agreement with an independent RF reference. Examples and
regression tests do not expand those validated limits either: a committed test
locks the behaviour it was written against, at the mesh, band and configuration
it was written for.
