...
| coaxial transmission-line reflection | broad-E5 physics demonstrated through `compute_coaxial_line_reflection(...)` inside the coax-line envelope, but the evidence artifacts are not committed, so a clean-checkout audit reports `coaxial_port` BLOCKED; older `compute_coaxial_s_matrix(...)` is deprecated/experimental |
| generalized planar and other advanced port workflows | planned or under active calibration and validation |
...
- `compute_coaxial_line_reflection(...)` is the coaxial line-reflection method; its broad-E5 physics was demonstrated inside its documented envelope, but the evidence artifacts are not committed, so a clean-checkout audit reports `coaxial_port` BLOCKED (do not cite `broad_e5_passed` until artifacts are committed and the auditor returns PASSED); `compute_coaxial_s_matrix(...)` remains deprecated/experimental.
- Floquet surfaces are experimental and should not be presented as a default S-parameter path.
