---
title: "Reference Lane"
description: "Public summary of the current recommended rfx lane and experimental surfaces."
sidebar:
  order: 41
---

## Recommended default lane

Start with the **uniform Cartesian Yee RF/FDTD lane**:

- PEC, CPML, or bounded UPML boundaries
- point/current sources
- lumped and wire ports
- specialized microstrip-line and rectangular waveguide workflows where documented
- probes, flux monitors, Harminv resonance extraction, selected S-parameter workflows, and benchmarked far-field workflows

This is the lane to use first for ordinary RF examples and public tutorials.

## Non-default lanes

The following surfaces are real and useful, but should be treated as narrower than the default lane:

| Surface | Public status |
|---|---|
| non-uniform mesh | shadow thin-substrate lane; retained but not the default claim lane |
| distributed execution | scaling lane; check feature combinations before use |
| Floquet/Bloch | experimental periodic/unit-cell workflow |
| SBP-SAT subgridding | research/experimental local mesh refinement |
| coaxial transmission-line reflection | validated only through `compute_coaxial_line_reflection(...)` inside the coax-line envelope; older `compute_coaxial_s_matrix(...)` is deprecated/experimental |
| generalized planar and other advanced port workflows | planned or under active calibration and validation |
| inverse-design extensions | available for selected workflows; check examples and limitations |

## S-parameter API boundaries

- `run(compute_s_params=True)` is the full S-matrix API for lumped/wire `add_port(...)` workflows.
- `compute_msl_s_matrix()` is the microstrip-line S-matrix API.
- `compute_waveguide_s_matrix()` is the rectangular waveguide S-matrix API.
- `compute_coaxial_line_reflection(...)` is the current validated coaxial line-reflection method inside its documented envelope; `compute_coaxial_s_matrix(...)` remains deprecated/experimental.
- Floquet surfaces are experimental and should not be presented as a default S-parameter path.

## Practical rule

If a feature is not in the recommended default lane, call it shadow,
experimental, or under active validation as appropriate and point users to the
current example or limitation page.
