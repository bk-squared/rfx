...
`calculator="waveguide"` also runs two setup audits that speak in input units — the record length against the far-boundary round trip per port (`record_shorter_than_far_boundary_round_trip`, error severity below one round trip), and the mirror covariance of every plane index an opposite-direction port pair uses (`port_index_mirror_asymmetry`). A healthy two-port guide still carries the informational `port_index_mirror_known_e_plane_offset` note, so read `report.ok`, not `len(report)`.
