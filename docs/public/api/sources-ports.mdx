...
Eager `compute_msl_s_matrix()` results include
`MSLSMatrixResult.reliable` with shape `(n_ports, n_freqs)`. A false entry marks
a standing-wave-null bin where both port voltage and current are too small for
a reliable single-run wave split; the numeric `S` value remains present but is
not valid physical data for that bin.
