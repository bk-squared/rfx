...
## Studio and experiment workflow boundary

Studio, the experiment CLI, and the MCP control plane provide versioned specs,
preflight, isolated CPU runs, immutable revisions, comparisons, studies, and
replay bundles. Their success states establish workflow completion and
provenance, not a broader electromagnetic accuracy claim. The observable still
inherits the mesh, source, port-family, and validation envelope documented on
this page. MCP writes and costly actions additionally require the server-side
approval flow described in [Studio and Durable Experiments](/rfx/guide/studio-experiments/).
