...
| `add_waveguide_port(...)` | `compute_waveguide_s_matrix()` | rectangular waveguide workflow; use `normalize="flux"` where the guide recommends it |
| `add_coaxial_port(...)` | `compute_coaxial_line_reflection(...)` | validated one-port coaxial transmission-line reflection envelope; the older `compute_coaxial_s_matrix(...)` path is deprecated/experimental |
...
| coaxial line reflection | validated only inside the documented transmission-line reflection envelope |
| generalized planar / other advanced ports | planned or under active validation |
