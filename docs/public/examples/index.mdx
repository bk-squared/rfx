---
title: "Public Examples"
description: "Recommended public example entry points for rfx."
sidebar:
  order: 30
---

Use these examples as starting points, not as exhaustive coverage of every feature.

## Recommended entry points

| Page or script | Why it matters |
|---|---|
| [Crossval First](./crossval/) | current public cross-check entry point |
| [RF Workflows](./rf-workflows/) | practical workflow framing |
| `examples/crossval/05_patch_antenna.py` | current patch antenna cross-check |
| `examples/crossval/11_waveguide_port_wr90.py` | rectangular waveguide-port workflow |
| `examples/nonuniform_patch_demo.py` | non-uniform mesh demo; shadow lane |
| `examples/inverse_design/multilayer_ar_coating.py` | conservative inverse-design demo |

## How to read the example surface

- Start with uniform Yee RF examples when you want the safest path.
- Treat non-uniform examples as shadow unless a guide says otherwise; treat
  distributed, Floquet/Bloch, general subgridding, generalized planar ports,
  and advanced inverse-design examples as experimental unless a guide says
  otherwise. Coaxial line-reflection claims must stay inside the documented
  one-port transmission-line envelope. The subgridding guide documents the
  narrow guarded one-sided production envelope.
- Prefer current scripts in `examples/crossval/` over older narrative snippets.
