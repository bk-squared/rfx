...
rfx is a 3-D finite-difference time-domain (FDTD) electromagnetic simulator built in JAX.
...
A capability listed neither here nor on [Support Boundaries](/rfx/api/support-boundaries/) is outside the documented support scope.
...
sim.add_source(
    ...
    amplitude_kind="current",   # amperes; required from 1.9, the default from 2.0
)
...
- [Cross-Validation & Accuracy](guide/validation/) — which page carries which kind of validation evidence
