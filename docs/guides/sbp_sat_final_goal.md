...
The immediate lane is now private plane-wave source request/spec adapter design
before subgrid-vacuum parity. The private solver-local skew E/H helper is
...
ladder preserves the failing boundary-expanded baseline. The previous analytic
source lanes isolated the old sheet-source phase-front failure, recorded
...
follow-up fixture-path wiring lane now records
`private_plane_wave_fixture_path_wiring_blocked_no_public_promotion` because no
safe private request/spec adapter carries the W1/R1 contract into the
subgrid-vacuum fixture path without forbidden runner/API promotion. The goal
gate therefore remains claims-closed until a private adapter can score unchanged
vacuum-stability thresholds and later true R/T readiness proves the hunk
