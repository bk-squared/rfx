...
| `add_coaxial_port` + `compute_coaxial_line_reflection(...)` | Coaxial one-port line-reflection path. M74 broad-E5 physics was demonstrated (analytic Γ envelope over short/open/matched/resistive loads, two characteristic impedances, mesh-resolution sweep) plus independent MEEP broad-E4 short/open comparison over 4--12 GHz. However, all evidence artifacts live in gitignored `.omx/`, not committed; a clean-checkout audit (`check_port_external_references.py`) reports `coaxial_port` BLOCKED. Re-validation is pending the validation-framework rework. Keep claims inside the stated coaxial transmission-line reflection physics envelope, but do not cite `broad_e5_passed` until artifacts are committed and the auditor returns PASSED. |
