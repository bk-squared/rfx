...
| `add_waveguide_port` | Strong current port-family surface because empty-guide, PEC-short, passivity, Airy, dump, external-solver, and flux-era envelope artifacts exist. Documentation must cite exact gates and keep clean-checkout artifact availability reconciled. **The external-solver leg is PROVENANCE-DISPUTED as of 2026-08-31 (#812 Phase 0):** `tests/fixtures/waveguide_broad_e5/wr90_rectangular_broad_e4_comparison.json` names `/tmp/cv11_fresh.stdout` as its source and its rfx leg rebuilds to `0.0186`--`0.0194` slab-`S11` `max_mag_abs_diff` from every committed cv11 stdout against its own recorded `0.0707`. Under the reporting format below it fails the artifact-path/exact-command requirement, so cite the analytic-Airy and envelope legs — not that E4 leg — until it is settled. See `docs/design_notes/20260831_cv11_broad_e4_artifact_provenance.md`. |
