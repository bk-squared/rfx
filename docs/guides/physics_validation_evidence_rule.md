...
| `add_msl_port` | **E5-narrow / eigenmode-blocked** for the documented uniform Laplace/quasi-TEM thru and notch configurations. The analytic MSL notch filter gate has a committed post-#511/#507 run, and the matched-geometry OpenEMS notch comparison is characterized rather than tight. Nonuniform Laplace mode has internal regression coverage only, and eigenmode remains unsupported. Use `docs/guides/sparameter_support_matrix.md` for the exact envelope and current metrics. |
