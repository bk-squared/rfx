...
| Coax-to-microstrip transition | **experimental** | `compute_coax_msl_transition(...)` is a diagnostic cross-family calculation, not a validated transition. The microstrip side's power-wave normalization over-reads power by about a factor of three, so the returned matrix is not passive; the call returns it with a `UserWarning` unless `strict_passivity=True`, which raises instead. No magnitude from this path is usable as an absolute power reference, and the lane is not scheduled for correction before 2.0. For a result you can cite, use `compute_msl_s_matrix(...)` and the coaxial lanes separately. Listed in [known limitations](known_limitations.md). |
