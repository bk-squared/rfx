...
| Rectangular-waveguide S-matrix | **limited** | Uniform single-mode magnitude results are validated across the documented WR-band tests. Use `compute_waveguide_s_matrix(...)`; phase, junction, and nonuniform results have narrower evidence. Differentiation through `eps_override` / `sigma_override` runs on `normalize=False` and `normalize="flux"` only; `normalize=True` is host-assembled and raises `NotImplementedError` for a traced override. Multimode (`n_modes>1`) is host-assembled and outside the differentiable chain. S is a modal voltage-wave ratio referenced to each port's own discrete modal impedance, so it is a power-wave S only when all ports share one cross-section. The `normalize="flux"` result dtype follows `JAX_ENABLE_X64` (complex64 by default, complex128 under x64), as `normalize=False` does. **Chain battery: chain-closed (v1.8) for uniform single-mode S on the `normalize=False` and `normalize="flux"` lanes.** The three runs below are PRE-2.0 measurements. The PEC short is a conductor body, so its far face moves with the ownership contract (#931) and every verdict below was measured under the old realization; they stand as dated history, and the battery must be re-run and its artifacts recaptured before this row quotes a 2.0 number. The magnitude legs are the falsifier that the change is confined to bodies — a short reflects at its FRONT wall either way — and the guide-width measurement is the leg that was expected to move (the #868 "40 mm guide reads 42 mm" class). Three pre-declared WR-90 runs of the battery in [`chain_closure_contract.md`](../design_notes/chain_closure_contract.md) on rungs `a/9`, `a/18`, `a/36` (the claims rung is `a/36`, 0.635 mm; the coarser two are report-only), DUTs thru (the control that shows the gates are not vacuous) / PEC short / `eps_r = 4` slab, 185 stored verdicts, no gate or pin moved between runs: run 1 (VESSL run 369367257823, artifact `fixture.json`, retained for history and no longer gated) measured the N+1-cell port and left 24 red in four families (#868, #869); run 2 (VESSL run 369367258205, PR #893, `fixture_guide_cell_aperture.json`) measured the corrected port and left 9 red — the 8 `normalize="flux"` forward-identity legs (float32 reassociation of the Poynting DFT under the tape) and the zero-derivative AD leg; run 3 (VESSL run 369367258638, `fixture_v18_close.json`, the artifact the replay test gates against) reads contract criterion 1 (forward identity: the reverse-mode-traced result equals the untraced one) and 3(a) (AD-vs-FD) under x64 (JAX 64-bit mode) on the flux lane per the pre-declaration of 2026-09-05 (`docs/design_notes/20260905_v18_close_predeclaration.md`), the forward default still float32 and the float32 reading stored beside every leg: **134 pass / 51 report_only / 0 fail / 0 not interpretable** — The 51 report_only verdicts are the 36 coarse- and mid-rung physics gates and referees (report-only below the claims rung by pre-declaration), the 12 ungated power-closure keys, and the 3 zero-derivative legs (1 AD-vs-FD, 2 gradient-invariance); the 134 pass are the claims-rung gates, the AD legs, the plane-shift and ladder gates. Criterion 1: forward identity at most 1.051e-8 of the rtol 1e-5 / atol 1e-7 allowance on the 8 flux legs (1.0 sits at the bound), exactly 0 on the 8 `normalize=False` legs; the float32 reading of the flux legs stays run 2's red (1.083 / 1.515 / 1.761 of the allowance), so a float32 gradient pipeline on that lane is outside the declaration. Criterion 2, at the claims rung: column power max 1.000353 against 1.02, complex reciprocity at most 4.81e-3 against 0.01, ring-down witness -100.9 to -102.6 dB per drive; the coarse and mid rungs are report-only, and the coarse slab reads column power 1.00711 and complex reciprocity 0.0309, three times the 0.01 gate. Criterion 3(a): 15 of 16 AD-vs-FD legs pass at rel 1.22e-4 to 1.07e-2 against 0.05; the pre-declared zero-derivative leg (PEC-short |S11|^2 under `eps_override`, flux) is report-only — x64 AD -2.943e-7 and FD -5.154e-8 share the sign and sit a factor 5.7 apart, outside the factor-3 band. 3(b): |S| invariant to 1.76e-7; gradient invariance pinned at 0.001 from a 2.33e-7 envelope over the ten pinned legs (float32 both sides), the two zero-derivative legs (2.7e-4, 6.5e-3) report-only on the same pre-declared exit as 3(a); rotation 0.0317 degrees against the Yee beta (gate 3 degrees), wrong-sign discriminator 64.05 degrees against a 10 degree floor. 3(c): 10 of 10 ladders interpretable and non-increasing. 3(d), at the claims rung: PEC-short |S11| within 0.999715--1.000143 (`normalize=False`) and 0.999995--1.000007 (flux) against 0.99--1.03; slab vs analytic Airy 0.01262 / 0.00903 in magnitude against 0.05 and 6.10 / 6.15 degrees in phase against 15.0 degrees; plus the five broad-E5 replay bands (WR-340, WR-62, WR-28, WR-15, WR-10). The coarse rung is report-only and outside both referee gates (slab vs Airy 0.1487 in magnitude, 16.94 degrees in phase). The declaration's own falsifier holds: the same AD stage with the float32 primary reproduces run 2's 9 red exactly. Fences: phase referee = analytic Airy only; junctions, multimode and nonuniform meshes excluded; power closure bounded, not demonstrated (interior-flux witness within 2.146e-05 of the port route at the float32 noise floor, independent in the plane index only, PR #870); accuracy statement for the `normalize=False` lane (issue #873, closed 2026-09-21 as a stated accuracy, not a defect): on an EMPTY guide it reads column power 1.0061 / 1.0012 / 1.0003 at `a/9`, `a/18`, `a/36` (0.027 / 0.005 / 0.001 dB, `fixture_v18_close.json`; run 1's 1.825e-2 belonged to the N+1-cell port of #868), second order in `dx` — the port's mode templates are cell-centred while `Ez`/`Hy` are node-registered in the broad-wall direction, so the driven port launches a TE20 near field and reads it back as TE10 ([results note](../design_notes/waveguide_driven_plane_near_field_composition_results.md)); not gated; "chain-closed" is not "supported" — the row stays limited until `ROADMAP.md` redefines the matrix. Artifacts: `tests/oracle/test_waveguide_chain_battery{,_guide_cell_aperture,_v18_close}.py`, `tests/unit/geometry/test_waveguide_chain_battery_geometry.py`, `tests/fixtures/waveguide_chain_battery/` (three artifacts + README), the three pre-declarations `waveguide_chain_battery_predeclaration.md` / `waveguide_chain_battery_remeasure_predeclaration.md` / `20260905_v18_close_predeclaration.md`, `scripts/diagnostics/waveguide_chain_battery_measure.py` and its three VESSL YAMLs. |
| Microstrip-line S-matrix | **limited** | Use `compute_msl_s_matrix(...)` with the laplace/quasi-TEM model, within its own documented envelope. `add_msl_port` accepts `+x`, `-x`, `+y`, and `-y`; the x-only eigenmode path remains outside the public support envelope. S uses positive real per-port power-wave references, exposed as `reference_impedances` separately from fitted Z0 and load resistance. See the [MSL power and replay contract](msl_power_contract.md). The external notch comparison is characterized rather than a tight cross-solver match. The extractor also reports a fitted `beta`; it is a diagnostic and does not enter S (`tests/unit/sparams/test_msl_fitted_beta_does_not_enter_s.py`). The fit recovers a known `beta` to 0.05 % (`tests/unit/sparams/test_msl_beta_fit_accuracy_envelope.py`). On a 600 µm trace over 250 µm of RO4350B, a float64 refit of the same probe phasors reads 1.32 … 1.33 % above the Hammerstad–Jensen closed form with five cells under the strip and 0.97 … 1.04 % with ten (every bin of 3.0–4.5 GHz); doubling the box at five cells moves it by +0.09 points, to 1.38 … 1.47 %, so box truncation does not account for the offset. The strip is one cell thick in each run, so the two cell sizes are also two conductor thicknesses, and no trend or limit value is claimed (`scripts/diagnostics/msl_phase_referee_dx_ladder/`). |
| Surface-impedance sheets | **limited** | `add_thin_conductor(..., surface_impedance_f0=...)` supports patterned shapes that are sheets — realized on ONE node plane, no thicker than one local cell along their normal — on its documented uniform and graded-mesh lanes. The lossy sheet's footprint and edge set are the PEC sheet's, so `f0` toggles loss and never geometry (#931). It is not a general dispersive or multi-runner conductor model. |
...
| Coax-to-microstrip transition | **experimental** | `compute_coax_msl_transition(...)` is a diagnostic cross-family calculation, not a validated transition. The microstrip side's power-wave normalization over-reads power by about a factor of three, so the returned matrix is not passive; the call refuses it by default (`strict_passivity=True`, raising `ValueError`), and `strict_passivity=False` returns the diagnostic matrix with a `UserWarning` instead. No magnitude from this path is usable as an absolute power reference, and the lane is not scheduled for correction before 2.0. For a result you can cite, use `compute_msl_s_matrix(...)` and the coaxial lanes separately, each within its own documented envelope. Listed in [known limitations](known_limitations.md). |
| Mixed lumped/wire + microstrip S-parameters | **experimental** | `compute_mixed_s_matrix(...)` is diagnostic and not in the validated set. Off-diagonal magnitudes come from Poynting flux and absolute magnitude has no external-solver comparison; off-diagonal phase mixes two reference-plane conventions. With `enforce_passivity=True` (the default) the returned diagonal is a jointly SVD-projected value, not the raw per-family measurement — read `S_raw` / `passivity_correction` for what was measured. Like the transition row above, this cross-family lane is not pursued further before 2.0. |
...
| Design-variable autodiff (`validation/research/multiband_nu/results/adq_*.json`; pre-declaration `docs/design_notes/20260913_nu_ad_designvar_predeclaration.md`) | `jax.grad` along PHYSICAL design variables — layer thickness at fixed total length and per-layer permittivity, through a fixed-topology stackup map (E4, PR #962) — judged by Taylor-remainder ORDER (AD-Q: R1 in [1.8, 2.2]; Lane 2b: R0 in [0.9, 1.1] and R1 >= 1.8, without an upper R1 cutoff) and by central FD inside three times the FD's own estimated error, not by a flat percentage. Revert-proof: the same test re-run with the CFL time step cut out of the gradient (`stop_gradient(dt)`) | Transient 120-step loss: h_core_left, **h_thin**, h_core_right, eps_core_right verified; h_thin R1 2.047 and FD inside a **2.8e-4** band; together they cover **88.3 %** of the cell-space gradient norm. **The dt path is verified, not assumed**: with it removed the h_thin test FIRES (R1 0.947); its share of the thickness gradient is 18.5 %. Resonant 8000-step DFT-power loss: eps_core_left/right and eps_air verified; **thickness NOT verified on that observable** — FD fires, the observable is rippled at ~1 % in thickness (central FD within -1.3..+4.5 % of AD for steps <= 0.4 %). Lane 2b (`results/e7_zsmooth.json`; `docs/design_notes/20260915_nu_lane2b_predeclaration.md`): the same stack and 8000 steps with a Hann-windowed comb spectral loss — **h_thin verified** (R1 1.981, FD inside a 7.8e-4 band, floor 3.6 ulp; the dt revert-proof fires, share -1.045), h_core_left / h_air verified; h_core_right FIRED on the one-sided R1 (1.721) along the exact negative of h_core_left's direction (HELD, 2.188) — diagnosed from the ladders as the statistic's cubic asymmetry (even remainder 1.99-2.01 on all four controls, central remainder h^3, gradient error along the line < 6.8e-4), recorded as FIRED. In-plane (E6 + Lane 2b `e7_y.json` / `e7_pos.json`): band **width** verified on x and on y (R1 1.94-1.96, dt share 0.59 / 0.77, revert fires); band **position** verified on x with the probe on the source side (R1 2.233 / 1.989, FD 6.7e-4 / 7.5e-3); with the probe across the strip (E6, y) position stays INCONCLUSIVE on order (lead/tail cancellation) and FD-supported. Per-cell gradients on tied-minimum cells are an equal-split subgradient (`g_ad = FD+ + (FD- - FD+)/n_tied`), not a derivative; F-S5's per-cell 15 % figure above is a smoke check, not this claim |
...
- **In-plane grading is COVERED for three witness classes, and only
  ...
  - *Design-variable autodiff (E6, `docs/design_notes/20260913_nu_lane2_inplane_designvar_ad_predeclaration.md`):*
    ...
    Lane 2b (`20260915_nu_lane2b_predeclaration.md`): the same on y (R1
    1.943 / 1.962, dt share 0.59 / 0.77); band position order-verified on
    x with the probe on the source side (R1 2.233 / 1.989), FD-supported
    only with the probe across the strip (E6, y).
  ...
  E6 and E7 use 0.5 mm, and the E5 relabel control is unpinned with matching ends.
