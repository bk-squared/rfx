...
| Microstrip-line S-matrix | **limited** | Use `compute_msl_s_matrix(...)` with the laplace/quasi-TEM model, within its own documented envelope. `add_msl_port` accepts `+x`, `-x`, `+y`, and `-y`; the x-only eigenmode path remains outside the public support envelope. S uses positive real per-port power-wave references, exposed as `reference_impedances` separately from fitted Z0 and load resistance. See the [MSL power and replay contract](msl_power_contract.md). The external notch comparison is characterized rather than a tight cross-solver match. Accuracy statement for the fitted `beta` (issue #830, closed 2026-09-21 as a stated accuracy): with the current fixture a replay against historical openEMS fields (not a matched-board run) reads rfx's `beta` at most 1.41 % from the Hammerstad-Jensen closed form of the board it realizes; the last matched run (2026-08-27, pre-2.0) reads +0.87 % to +0.94 % on every gated bin of 3.0-4.5 GHz, openEMS -0.31 % to +0.08 %. The offset is unattributed. `beta` is a fitted diagnostic and does not enter S: the through phase `angle(S21)` differs from openEMS by at most 0.34 degrees against a 3-degree gate on the matched run, 0.03 % to 1.1 % of the electrical length and largest at the low band edge. One mesh (`dx = 50 um`), no tracked refinement; details in the [S-parameter matrix](sparameter_support_matrix.md). |
