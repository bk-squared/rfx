...
| Design-variable autodiff (`validation/research/multiband_nu/results/adq_*.json`; pre-declaration `docs/design_notes/20260913_nu_ad_designvar_predeclaration.md`) | `jax.grad` along PHYSICAL design variables — layer thickness at fixed total length and per-layer permittivity, through a fixed-topology stackup map (E4, PR #962) — judged by Taylor-remainder ORDER (R1 slope in [1.8, 2.2]) and by central FD inside three times the FD's own estimated error, not by a flat percentage. Revert-proof: the same test re-run with the CFL time step cut out of the gradient (`stop_gradient(dt)`) | Transient 120-step loss: h_core_left, **h_thin**, h_core_right, eps_core_right verified; h_thin R1 2.047 and FD inside a **2.8e-4** band; together they cover **88.3 %** of the cell-space gradient norm. **The dt path is verified, not assumed**: with it removed the h_thin test FIRES (R1 0.947); its share of the thickness gradient is 18.5 %. Resonant 8000-step DFT-power loss: eps_core_left/right and eps_air verified; **thickness NOT verified** — FD fires, the observable is rippled at ~1 % in thickness (central FD within -1.3..+4.5 % of AD for steps <= 0.4 %). Per-cell gradients on tied-minimum cells are an equal-split subgradient (`g_ad = FD+ + (FD- - FD+)/n_tied`), not a derivative; F-S5's per-cell 15 % figure above is a smoke check, not this claim |
