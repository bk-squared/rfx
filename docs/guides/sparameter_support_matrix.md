...
external openEMS referee (VESSL run-3 `369367251629` and the first
default-scale green promoted-lane run VESSL `369367252220`) brackets — it does not judge — this method's own
...
the whole referee: it imported no rfx module and read no rfx fixture, so
**no leg inside it was E4**; the E4 in this method's evidence
