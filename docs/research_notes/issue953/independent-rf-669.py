"""Independent NumPy reconstruction of retained669 V/I and spectral predicates."""
...
DATA = Path(__file__).resolve().parent / 'gpu-369367260669/artifacts'
...
for label in LABELS:
    ...
    selected=sorted(set([i-1,i,i+1]+sum(variants['projected']['crossing_bin_indices'].values(),[])))
...
for variant in ('projected','raw_saved','raw_reconstructed'):
    b=rows['baseline']['variants'][variant];one=rows['stub_1cell']['variants'][variant];narrow=rows['stub_narrow']['variants'][variant]
    ...
    conditions[variant]=dict(baseline_G1=b['G1'],baseline_all_existing_gates=bool(b['G1'] and b['G2'] and b['G3'] and b['depth_witness'] and rows['baseline']['G4_port0']),
                            ...
                            one_cell_visible=bool(abs(delta)>=.5*abs(prediction)),narrow_G2_fails=not narrow['G2'],narrow_depth_pass=narrow['depth_witness'])

report=dict(scope='Independent retained669 V/I reconstruction and spectral predicates; no production replay/estimator helper imports, no field evolution.',
            ...
            geometry_and_observation_limits=['One-cell arm ny=279 versus baseline280.', 'Narrow automatic probe0 indices[100,451] versus baseline/one-cell[99,452].',
                                             'These are approved production-builder arms, not an otherwise identical coordinate-locked causal A/B.'],
            ...
            raw_artifact_sha256={label:hashlib.sha256((DATA/f'{label}-raw-vi.npz').read_bytes()).hexdigest() for label in LABELS})
out=args.out
