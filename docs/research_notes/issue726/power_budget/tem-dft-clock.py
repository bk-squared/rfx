...
with enable_x64():
    ...
    idx = 5
    material = MaterialArrays(jnp.ones(grid.shape, dtype=jnp.float64),
                             jnp.zeros(grid.shape, dtype=jnp.float64),
                             jnp.ones(grid.shape, dtype=jnp.float64))
    ...
    for mode in ["forward", "backward", "standing"]:
        ...
        state0 = init_state(grid.shape, field_dtype=jnp.float64)._replace(
        ...
        with patch.object(engine, "init_state", return_value=state0):
            result = engine.run(grid, material, 4*period_steps, boundary="pec",
                                ...
                                dft_planes=probes, field_dtype=jnp.float64)
        ...
        assert record["relative_error"] < 1e-11, record
