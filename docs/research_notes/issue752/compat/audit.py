...
root = Path('/root/rfx-codex/.git/issue752/compat')
records = []
real_grad = jax.grad
...
with patch.object(jax, 'grad', observe_grad):
    result = pytest.main([
        ...
        'tests/unit/ports/test_msl_preflight_conductor_gap.py',
        'tests/unit/sparams/test_msl_z0_bias_floor_sweep_realized_anchor.py',
        '-q', '-s', '-o', 'addopts=',
    ])
