...
CASES = (("h3", 254e-6/3), ("h4", 254e-6/4), ("h5", 254e-6/5),
         ("h6", 254e-6/6), ("dx80", 80e-6), ("dx60", 60e-6))
PERIODS = 20.0
...
def build_case(label):
    ...
    yc = (y0+y1)/2
    margin = math.ceil(.002/dx)
    feeds = (margin*dx, (nx-margin)*dx)
    offset, spacing = math.ceil(.006/dx), math.ceil(.003/dx)
    sim = Simulation(freq_max=5e9, domain=(nx*dx, ny*dx, nz*dx), dx=dx,
                     cpml_layers=8, boundary=BoundarySpec(
                         x="cpml", y="cpml", z=Boundary(lo="pec", hi="cpml")))
    sim.add_material("substrate", eps_r=3.66)
    sim.add(Box((0., 0., 0.), (nx*dx, ny*dx, h)), material="substrate")
    sim.add(Box((0., y0, h), (nx*dx, y1, h)), material="pec")
    for index, (feed, direction) in enumerate(zip(feeds, ("+x", "-x"))):
        sim.add_msl_port(position=(feed, yc, 0.), width=w, height=h,
                         direction=direction, impedance=50., name=f"p{index}",
                         n_probe_offset=offset, n_probe_spacing=spacing,
                         n_probes=5, eps_r_sub=3.66,
    ...
    xs = [list(map(float, msl_probe_x_coords_n(grid, msl_port_from_entry(pe),
          pe.n_probes, pe.n_probe_offset, pe.n_probe_spacing))) for pe in sim._msl_ports]
    ...
    plan = dict(label=label, desired_width_m=600e-6, desired_height_m=254e-6,
                dx_m=dx, height_m=h, width_m=actual_width, trace_thickness_m=0.,
                height_intervals=nh, width_intervals=nw, domain_m=list(sim._domain),
                grid_shape=list(grid.shape), dt_s=float(grid.dt),
                n_steps=int(grid.num_timesteps(num_periods=PERIODS)),
                num_periods=PERIODS, actual_duration_s=int(grid.num_timesteps(num_periods=PERIODS))*float(grid.dt),
                pulse_f0_hz=3.75e9, pulse_bandwidth=0.8,
                eps_r=3.66, feed_positions_m=list(feeds),
                probe_positions_m=xs, port_definitions=[asdict(p) for p in sim._msl_ports],
                repo_simplified_reference_ohm=legacy, repo_eps_eff=ee,
                hj1980_reference_ohm=hj, hj1980_eps_eff=hj_ee,
                material_signatures={name:array_signature(value) for name,value in
                                     zip(materials._fields, materials)},
...
def run_case(label, out, *, dry_run=False):
    ...
    plan.update(driver_sha256=hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
                python=sys.version, jax=jax.__version__, backend=jax.default_backend(),
                x64=bool(jax.config.x64_enabled), frequencies_hz=FREQUENCIES.tolist())
    ...
    def recorded(*args, **kwargs):
        ...
        for name in ("eps_r", "mu_r"):
            assert array_signature(getattr(mats, name)) == plan["material_signatures"][name]
        ...
        call = dict(drive=drive, n_steps=int(inp["n_steps"]),
                    material_signatures={n:array_signature(v) for n,v in zip(mats._fields,mats)},
                    probes=[p._asdict() for p in inp["probes"]],
                    sources=[dict(i=p.i, j=p.j, k=p.k, component=p.component,
                                  waveform=array_signature(p.waveform)) for p in inp["sources"]],
                    boundary=inp["boundary"], cpml_axes=inp["cpml_axes"],
                    pec_axes=inp["pec_axes"], periodic=inp["periodic"],
                    dft_planes=[dict(component=p.component, axis=p.axis, index=p.index,
                                     region=p.region, freqs_hz=np.asarray(p.freqs).tolist(),
                                     total_steps=p.total_steps, window=p.window,
                                     window_alpha=p.window_alpha) for p in inp["dft_planes"]])
        calls.append(call)
        ...
        inputs.update({f"source_{n}":np.asarray(p.waveform) for n,p in enumerate(inp["sources"])})
    ...
    try:
        with warnings.catch_warnings(record=True) as caught:
            ...
            result = sim.compute_msl_s_matrix(freqs=FREQUENCIES, num_periods=PERIODS,
                       enforce_passivity=False, raw_3probe_dump_path=str(out / "raw-vi.npz"),
                       report_every=5000)
        arrays = {n:np.asarray(getattr(result,n)) for n in (
            "S", "Z0", "beta", "freqs", "reliable", "settling_db", "cond_a", "beta_railed",
            "reference_impedances") if getattr(result,n) is not None}
        np.savez_compressed(out / "result.npz", **arrays)
        write_json(out / "diagnostics.json", dict(warnings=[str(w.message) for w in caught],
                   probe_clearance=[asdict(c) for c in result.probe_clearance],
                   assembly=result.assembly, completed_drives=len(calls)))
        assert len(calls) == 2
...
def main():
    ...
    if a.build_only:
        ...
        for label, _ in CASES:
            ...
            print(label, plan["height_m"], plan["width_m"], plan["grid_shape"], flush=True)
    else:
        if a.case is None:
