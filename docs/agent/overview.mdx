...
This Git-repository-public guidance is for external LLM agents; it is site-private
and excluded from `remilab.ai` navigation, export, deployment snapshots, and gitops.
...
5. **Record uncertainty.** If a workflow uses non-uniform mesh, subgridding,
   distributed execution, Floquet/Bloch, coaxial-line reflection, older coaxial
   S-matrix paths, or advanced inverse design, say whether it is supported,
   shadow, experimental, deprecated, or blocked.

## Core references

- [Quick Start](/rfx/guide/quickstart/)
- [Sources and Ports](/rfx/guide/sources-ports/)
- [Working on rfx — the coordination contract](./working-on-rfx.mdx)
- [Repo Map & Feature Discovery](./repo-map.mdx)
