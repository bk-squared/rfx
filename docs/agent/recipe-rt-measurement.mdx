...
`subtract_flux_monitors(sample, ref)` does exactly that, so nobody has to write
out the four private accumulator field names — and it raises `ValueError`
instead of broadcasting when the two monitors do not cover the same plane and
...
`validation/crossval/04_multilayer_fresnel.py` gates rfx against the analytic
...
Do not read those mean thresholds as an accuracy statement about the fringe
structure: a band mean over an interference pattern is blind to every zero-mean
shape error, and with the mean gates alone the case passed while the true slab
permittivity was 12.33% away from what the FDTD built (issue #812). Since #812
the script additionally gates each Fabry-Perot extremum of R(f) one at a time —
position against a window derived from the spectral bin and the exact Yee
dispersion relation (59/106/235 MHz at the 3.75/7.50/11.24 GHz fringes) and
value against 0.04 — and, when Meep is importable, gates Meep's numbers rather
than only its presence. If you need an accuracy figure for a slab, quote the
per-fringe deviations the script prints, not the band means.
...
- `validation/crossval/04_multilayer_fresnel.py` — dispersionless Fresnel slab,
