...
`tests/unit/sparams/test_normalization.py::test_normalized_s21_straight_waveguide`, which
...
That status is about magnitude and phase accuracy on the documented setups. The
chain-closure status is separate: the v1.8 chain battery measured every
criterion of [`chain_closure_contract.md`](../design_notes/chain_closure_contract.md)
in three pre-declared WR-90 runs, and the family is **chain-closed (v1.8)** on
the third (`tests/oracle/test_waveguide_chain_battery_v18_close.py`,
`tests/fixtures/waveguide_chain_battery/fixture_v18_close.json`, VESSL run
369367258638) — with criterion 1 (forward identity: the reverse-mode-traced
result equals the untraced one) and 3(a) (AD-vs-FD) read with JAX 64-bit mode
enabled on the `normalize="flux"` lane, the forward default still float32.
A float32 gradient pipeline on the flux lane is outside the declaration: the
float32 reading of the same legs is still red (forward identity 1.083–1.761
scaled against a bound of 1.0). Take gradients on that lane under
`JAX_ENABLE_X64=1`, or stay on `normalize=False`, which is bit-identical in
float32.
The support-matrix row carries the numbers and the scope fences (uniform
single-mode only; junctions, multimode and nonuniform meshes excluded); do not
read a green accuracy referee as a gradient-chain result, and do not read
chain-closed as "supported" — a stronger status this family does not carry.
...
The authoritative regression gates for the rectangular-waveguide port are
`tests/oracle/test_waveguide_port_validation_battery.py` and
`tests/unit/sparams/test_waveguide_twoport_contract_v1.py`. Read them for the
canonical set-up: empty guide, PEC short and a dielectric slab, with the
band-edge artifacts and the analytic phase-convention caveats stated there.
...
- rfx tests: `tests/unit/sparams/test_normalization.py` (baseline accuracy),
  `tests/oracle/test_conservation_laws.py` (reciprocity, unitarity)
