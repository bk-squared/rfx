...
`validation/crossval/02_ring_resonator.py` is the project-level ring resonator validation that uses `harminv` directly against Meep. It shows the full pattern: broadband `ModulatedGaussian` excitation, time-series extraction from `result.time_series` and `result.dt`, and the skip-fraction used to isolate the ringdown.
