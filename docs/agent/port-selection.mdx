...
| Microstrip / MSL | `add_msl_port` | `compute_msl_s_matrix` | E5-narrow / eigenmode-blocked | Use only within the characterized-not-tight MSL envelope in the [support contract](../guides/sparameter_support_matrix.md). The current MSL notch filter run is the post-#931 re-solve (dx = 63.5 µm = h_sub/4, issue #723; VESSL 369367259191, log `validation/crossval/_06b_notch_uniform_logs/20260907T124851Z_run.log`), with the trace and stub declared as sheets. It reports 2.16% notch-frequency error against the analytic notch evaluated on the 635.0 µm ELECTRICAL width (n_rows·dx; the sheet's GEOMETRIC node span is 571.5 µm), −39.4 dB depth, and median Re(Z0) 48.2 Ω, passing its listed gates; this does not broaden that envelope. Which width the quasi-TEM formula should take is NOT settled by that run: crossval-B pre-declared Z0 median 46.48 ± 1.0 Ω for the electrical width against ≈49.4 Ω for the geometric one, and the measurement landed at 48.2 Ω, between them — open with crossval-B, and no gate was moved for it. The same board realized as one-cell PEC Boxes read 1.40% / −43.3 dB / 46.5 Ω (2026-08-27 log): a pre-2.0 realization, history rather than current evidence. The earlier dx = 80 µm figures (1.63% / −34.2 dB / 57.9 Ω) are history — that mesh rasterized the declared 254 µm substrate to 320 µm. Eigenmode is unsupported (fenced); the non-uniform graded-mesh lane runs via the laplace feed (internally gated, external validation still owed). |
