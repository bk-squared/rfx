...
| Microstrip / MSL | `add_msl_port` | `compute_msl_s_matrix` | E5-narrow / eigenmode-blocked | Thru-line `\|S21\|` / `Z0` gate; notch error 1.63% and depth −34.2 dB (cv06b, regenerated 2026-08-09 on the #511/#507-corrected extractor, issue #519 — headline figures essentially unchanged from the pre-fix run because the V·I-split S11/S21 were unaffected by those fixes; Re(Z0) median 57.9 Ω in-range; run log `validation/crossval/_06b_notch_uniform_logs/`; the committed gate is looser: error <15%, depth <−10 dB). Eigenmode is unsupported (fenced); the non-uniform graded-mesh lane runs via the laplace feed (internally gated, external validation still owed). |
