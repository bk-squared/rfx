...
| Microstrip / MSL | `add_msl_port` | `compute_msl_s_matrix` | E5-narrow / eigenmode-blocked | Use only within the characterized-not-tight MSL envelope in the [support contract](../guides/sparameter_support_matrix.md). The post-#511/#507 cv06b run reports 1.63% notch-frequency error, −34.2 dB depth, and median Re(Z0) 57.9 Ω, passing its listed gates; this does not broaden that envelope. Eigenmode is unsupported (fenced); the non-uniform graded-mesh lane runs via the laplace feed (internally gated, external validation still owed). |
