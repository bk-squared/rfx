...
| Microstrip / MSL | `add_msl_port` | `compute_msl_s_matrix` | E5-narrow / eigenmode-blocked | Thru-line `\|S21\|` / `Z0` gate; notch error 1.63% and depth −34.3 dB (cv06b — single-run figures measured before the #511/#507 extractor fixes, not yet regenerated, superseded pending issue #519; the committed gate is looser: error <15%, depth <−10 dB). Eigenmode is unsupported (fenced); the non-uniform graded-mesh lane runs via the laplace feed (internally gated, external validation still owed). |
