...
(gate C4b, and `tests/crossval/test_spectral_feature_estimators.py`).
...
its gate keys moved with them and `tests/contracts/test_physics_gate_reporting.py` gained
...
in-test by `tests/crossval/test_cv06b_shallow_stub_model.py`. Prose that needs a
...
out (`tests/crossval/test_cv06b_build_falsifier_plumbing.py`) so a formatting crash
...
* The one shallow-notch row `tests/contracts/test_physics_gate_reporting.py` types into
