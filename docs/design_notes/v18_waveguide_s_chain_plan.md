...
`tests/crossval/test_waveguide_broad_e5.py`, zero run cost because they are replay). cv18,
...
**Scope.**
1. `tests/crossval/test_waveguide_broad_e5.py`: assert each fixture's `unitarity_min` /
...
`tests/crossval/test_waveguide_broad_e5.py`). They are replay gates, so widening 3(d) from one
