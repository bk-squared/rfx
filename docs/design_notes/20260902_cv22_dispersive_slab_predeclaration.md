...
## 15. Independent review (FIX-THEN-SHIP) — corrections, append-only; no claims-bearing number changes

Every claims-bearing number reproduced by the reviewer (TMM to 9e-16, ε(f) to 0 ulp, W_ADE, windows, §13 record arithmetic, 225 citations with 0 violations, ladder orders). The nine findings and what was done:

1. **§14.1 fitted tail rates were contaminated by the incident pulse.** The stored 300-step envelope of the Debye arm starts at step 808, 100 steps before `n_pulse_end` = 908, so its first two 50-step blocks (maxima 0.50, 0.51) are the pulse, not the ring-down; the Drude envelope (start 868) still holds the delayed transmitted pulse in its second block. `fit_tail_rate` now fits only whole blocks beginning at or after `n_pulse_end + TAIL_WINDOW` (one window of margin for the slab's group delay), the artifact records `tail.fit_start_step` and `tail.envelope_start_step`, and the committed r4 fitted keys were **recomputed from the STORED envelopes** (`22_dispersive_slab_fresnel.py --refit-tail-fits`, no rerun; `tail.fit_note` says so). Refitted, by key: Debye `validation/crossval/_22_dispersive_results/rfx.json::arms.debye.tail.fitted_rate_scat_refl_1_s = 8.8e9` / `validation/crossval/_22_dispersive_results/rfx.json::arms.debye.tail.fitted_rate_total_trans_1_s = 8.1e9` (`validation/crossval/_22_dispersive_results/rfx.json::arms.debye.tail.fitted_rate_blocks = 3` blocks) vs the derived `validation/crossval/_22_dispersive_results/rfx.json::arms.debye.run.record.rate_ring_1_s = 1.2e10` — §14.1's "missed by ~1.5×" survives (1.25–1.5×); Lorentz `validation/crossval/_22_dispersive_results/rfx.json::arms.lorentz.tail.fitted_rate_scat_refl_1_s = 7.1e9` / `validation/crossval/_22_dispersive_results/rfx.json::arms.lorentz.tail.fitted_rate_total_trans_1_s = 9e9` vs the material 7.33e9 (still close); Drude `validation/crossval/_22_dispersive_results/rfx.json::arms.drude.tail.fitted_rate_scat_refl_1_s = 1.3e10` / `validation/crossval/_22_dispersive_results/rfx.json::arms.drude.tail.fitted_rate_total_trans_1_s = 1.6e10` vs the material 9.42e9 and the 1.13 GHz etalon 1.27e10. **§14.1's Drude sentence is withdrawn:** "a faster component dominates its tail" was an estimator artefact of the pulse-contaminated fit; post-pulse, the Drude tail decays between the material pole and the sub-band etalon, as the derivation assumed. §14.1's six `fitted_rate_*` citations resolve against the artifact as committed at `631f0e0` (`git show 631f0e0:validation/crossval/_22_dispersive_results/rfx.json`), not against HEAD — stated here rather than edited there; a resolver run over this note at HEAD reports exactly those five stale values (the Debye transmitted-side fit happens to round the same).
...
9. **Meep res-40 margin, stated where the reference is chosen.** The 40 px/cm reference is converged only one rung under the window: Drude G4_mean_T `validation/crossval/_22_dispersive_results/rfx.json::arms.drude.meep.mean_dT_meep_tmm_gated = 0.0095` against `validation/crossval/_22_dispersive_results/rfx.json::arms.drude.meep.mean_window4_T = 0.017` is a 1.8× margin, and the 20 px/cm rung (`validation/crossval/_22_dispersive_results/meep_ladder_summary.json::arms.drude.rungs.20.mean_dT_meep_tmm_gated = 0.019`) would fail it. In §12.4's terms: the reference passes because it is the converged rung, not because the window has room for a coarser one; the manifest and README carry the same sentence.
