...
loses 35 to 46 dB of ring-down and three orders of magnitude of reciprocity (`alpha/TABLE.md`).
...
boundary-normal directions … cause PML to break down", and fields at glancing incidence decay slowly
in any PML. In both solvers what is inside the absorber is never ambiguous: it is what was drawn
...
- **kappa = 1.** (record: header of `tests/oracle/test_waveguide_port_validation_battery.py`) a
  ...
  formulation, and an earlier retune to 5 was retracted after the sweep was run. **Kept.**
...
535.43 cells and is realized as 536; its strip, declared to 34.0 mm, stops at cell 535, one cell
before the +x absorber and none before the −x one (`msl/EDGES.md`). The assembly check of #1136
(`assert_declared_span_is_filled`) is extended to conductors instead of excluding them.

**C4 — The absorber is mirror-symmetric.** A mirror-symmetric structure must give mirror-image fields.
On main the patch rig's conductor is mirror-symmetric, realized from the lo face node to the hi face
node (cell-centre sampling absorbs the rig's −0.1-cell nudge), and yet the growing wave held 0.66 of
the energy near +y against 0.002 near −y; with the magnetic absorber profiles sampled at the magnetic
field's own positions (PR #1012) it holds 0.34 and 0.31 (`dumps/n2_pad10_main/leader_profiles.txt`,
...
**A two-conductor line is ended by its port.** The segment of a line between a line port's termination
...
four lateral faces (380 to 1992 cells per face, read back from the arguments the time stepping
...
| ring-down witness | −97.1 → −57.8 dB | −98.4 → −63.4 dB |
...
the cause. The cause is the frequency-shift term: with the strip inside the absorber and `alpha`
scaled (`alpha/TABLE.md`, scaling read back from the profile arrays the time stepping received),

| alpha factor | ring-down | max \|S12 − S21\| | raw column power |
|---|---|---|---|
| 0.01 | −82.7 dB | 6.4e-4 | 1.003 |
| 0.25 | −74.7 dB | 4.7e-3 | 1.005 |
| 1 (shipped) | −63.4 dB | 5.1e-2 | 1.028 |
| 4 | −40.6 dB | 4.6e-1 | 1.37 |
| *as committed: strip ends at the absorber, port-terminated* | *−98.4 dB* | *6.5e-5* | *1.001* |
