...
| `04_multilayer_fresnel` | yes (partial, since 2026-09-10) | UPDATED 2026-09-11 (PR #974): the settling-extension loop grows `nx_interior` until the tails clear the family bar (settled at 800, 990 steps, run 369367260232, logs in `_04_fresnel_logs/`), and the lattice witness is GATED on that record and enters `rfx_self_ok`. The grow attempts are the rig variation, but only the settled rung is retained in `_04_fresnel_results/`; the pre-fix 719-step record survives only as `envelope.json` r1, still marked `carried-unwitnessed` because r1 was measured on the un-settled record (r2 is open under #928). Superseded text: "Its own rung does not settle; the lattice witness is reported, not gated." |
