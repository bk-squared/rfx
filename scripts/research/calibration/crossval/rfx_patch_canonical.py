...
Run (single resolution; caller submits to VESSL GPU):
    python scripts/research/calibration/crossval/rfx_patch_canonical.py --dx-mm 1.0 \
        --output scripts/research/calibration/crossval/out_canonical --n-steps 12000
    # finer GPU sweep:
    python scripts/research/calibration/crossval/rfx_patch_canonical.py --dx-mm 0.5 \
        --output scripts/research/calibration/crossval/out_canonical --n-steps 16000
...
def run_once(args):
    ...
    # 1.05 = per-bin passivity slack per tests/test_msl_port_integration.py:411
    # / issue80 PASSIVE_TOL family (do not invent a new tolerance).
