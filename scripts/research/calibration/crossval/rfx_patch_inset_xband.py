...
the domain with air+CPML below); the PR #272 post-mortem replaced it with the
repo's z_lo PEC boundary idiom. Its dx=197um "deep match" (-30.25 dB @
10.359 GHz) is a coarse-mesh miswired-frame artifact — see verdicts.json
(coarse-dip-depth). Kept for the inset-sweep history; do not extend.
...
Run (GPU):  python scripts/research/calibration/crossval/rfx_patch_inset_xband.py \
                --inset-list-mm 0,2.4,2.8,3.2,3.6 \
                --output scripts/research/calibration/crossval/out_xband
...
def build_sim(inset_m):
    ...
    # The former "thru" arm (inset_m < 0: one-port line run into the far CPML)
    # is deleted (#273): that wiring was the PR #272 miswiring — one port and
    # no matched second port gives no thru observable. The canonical
    # matched-thru is scripts/diagnostics/msl_thru_mesh_convergence.py (two
    # add_msl_port ends on a z_lo PEC ground boundary — the second port IS the
    # matched termination; pattern of tests/test_msl_port_integration.py).
    # Do not rebuild a duplicate thru fixture here.
    assert inset_m >= 0, "thru arm removed (#273); inset depth must be >= 0"
...
def run_one(inset_mm, args, out_dir):
    print("=" * 72)
    label = ("edge-fed reference" if inset_mm == 0
             else f"gap={NOTCH_GAP*1e3:.2f} mm")
