...
LOADS (degrades gracefully — compares whatever is present):
  (a) rfx mesh-sweep JSONs   scripts/crossval/out_gpu/*.json
  (b) rfx port-extent JSONs  scripts/crossval/out_gpu_port/ext*/*.json
  (c) openEMS reference      scripts/crossval/out_ref/openems_patch_s11.json
  (d) Balanis analytic f_res (from any JSON's analytic_resonance_hz)

PRODUCES:
  * overlay plot |S11| dB + phase vs f (one curve per solver/config)
        -> scripts/crossval/out_ref/compare_patch_s11.png
...
Run (works with only openEMS+analytic present; fills in rfx when JSONs land):
  python scripts/crossval/compare_patch_s11.py
"""
