...
Run (GPU):
    python scripts/crossval/rfx_patch_inset_msl.py \
        --inset-list-mm 10,12,14,16 --output scripts/crossval/out_inset
"""
...
def build_sim(inset_cells, pulse_f0, pulse_bw):
    ...
    thru = inset_cells is None
    x_conn = None if thru else X_PATCH_LO + C(inset_cells)
    ...
    if thru:
        # THRU sanity: 50-ohm line across the whole domain into the far CPML.
        # A healthy msl port + extractor must show LOW, smooth |S11| here.
        sim.add(Box((0, Y_FEED_LO, Z_MET_LO), (DOM_X, Y_FEED_HI, Z_MET_HI)),
                material="pec")
        sim.add_msl_port(
...
def run_thru(args, out_dir):
    import jax.numpy as jnp
    print("=" * 72)
    print("THRU sanity (no patch): 50-ohm line into the far CPML")
    sim = build_sim(None, args.pulse_f0_ghz * 1e9, args.pulse_bw)
    sim.preflight(strict=False)
    t0 = time.time()
    res = sim.compute_msl_s_matrix(
        freqs=jnp.linspace(args.freq_lo_ghz * 1e9, args.freq_hi_ghz * 1e9,
    ...
    print(f"  THRU |S11|: max {db.max():.2f} dB, mean {db.mean():.2f} dB, "
          f"min {db.min():.2f} dB  ({t_run:.0f}s)")
    payload = {"case": "thru_sanity", "freqs_hz": freqs.tolist(),
               "s11": [[float(v.real), float(v.imag)] for v in s11],
               "runtime_s": t_run}
    with open(os.path.join(out_dir, f"rfx_patch_inset_thru{args.tag}.json"), "w") as fh:
        json.dump(payload, fh)
    return None
...
def main():
    ...
    insets = [None if v.strip().lower() == "thru" else float(v)
              for v in args.inset_list_mm.split(",") if v.strip()]
    results = [r for r in (run_one(d, args, args.output) for d in insets)
               if r is not None]
