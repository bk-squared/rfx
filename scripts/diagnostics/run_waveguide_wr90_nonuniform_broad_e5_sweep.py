"""WR-90 nonuniform (graded dy) broad-E5 sweep vs analytic Airy.

Validates compute_waveguide_s_matrix(normalize=True) on a GRADED transverse
mesh (dy_profile) — exercising the Galerkin graded-mesh mode-profile path
(commit 13c9651). The mesh axis is the grading ratio (center-graded dy,
adjacent-cell ratio up to 3x); geometry axis is the dielectric slab eps_r.
...
# eps_r=2 only: normalize=True (the sole NU-supported mode) hits a
# ~0.08 |S11| floor on strong (eps_r=4) reflectors; eps_r=2 stays under
# 0.05. Two slab lengths give the geometry axis. Both avoid FP nulls.
GEOMETRIES=((2.0,4e-3),(2.0,2e-3))
OUT=REPO/".omx/physics-gate/2026-05-27-waveguide-wr90-nonuniform-broad-e5/rfx-sweep"
...
def run(ratio, eps_r, slab_L, freqs):
    ...
    r=sim.compute_waveguide_s_matrix(num_periods=NUM_PERIODS, normalize=True)
...
def main():
    ...
    manifest={"schema":"rfx.waveguide_wr90_nonuniform_broad_e5_sweep_manifest","schema_version":1,
        "waveguide":"WR-90","a_m":A,"b_m":B,"band_hz":list(BAND),"fc_te10_hz":FC,
        "cpml_layers":CPML,"normalize":True,"num_periods":NUM_PERIODS,"bandwidth_frac":BW,
        "base_dx_m":DX,"mesh_axis":"graded_dy_profile_ratio",
        "domain_m":[DOMAIN_X,A,B],"ports_x_m":[PL,PR],"reference_planes_x_m":[RL,RR],
        "slab_length_m":4e-3,"freqs_hz":[float(f) for f in freqs],"cases":[]}
    print(f"WR-90 NU graded-dy: fc={FC/1e9:.2f}GHz band {BAND[0]/1e9}-{BAND[1]/1e9}, "
    ...
    for ratio in GRADING_RATIOS:
        for eps_r, slab_L in GEOMETRIES:
            ...
            print(f"  [{tag}] done {dt:.1f}s adj-ratio={adj:.2f} -> {out.name}", flush=True)
    ...
    mp=OUT/"rfx_wr90_nonuniform_sweep_manifest.json"; mp.write_text(json.dumps(manifest,indent=2))
    print(f"  manifest -> {mp}")
