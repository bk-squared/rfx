...
def main():
    if sys.argv[1] == 'json':
        records = {f:compact_record(OUT / f'{f}_dry/baseline/assembly_received_00.json') for f in ('cv06b','cv20')}
        for f,d in records.items():
            d['status'] = json.loads((OUT / f'{f}_dry/baseline/status.json').read_text())
        with (OUT/'edges_today.json').open('x') as stream:
            json.dump(records, stream, indent=2)
            stream.write('\n')
    elif sys.argv[1] == 'md':
        records = json.loads((OUT/'edges_today.json').read_text())
        text = 'Readback: `rfx.simulation.run`; time-stepping calls = 0. Indices are zero-based.\n\n'
        text += '\n'.join(render_record(f, d) for f,d in records.items())
        with (OUT/'EDGES.md').open('x') as stream:
            stream.write(text)
