...
def main():
    ...
    lines += [table(['fixture','A1','A2','A3 named script','rfx-only copied builder/call'],rows),
              ...
              'Captured diagnostics: all 14 `MSLSMatrixResult` fields per run in `diagnostics.json`; numeric fields in `diagnostics.npz`; complex S and frequencies in `s.npz`; preflight text verbatim in `preflight.txt` and public-call preflight/warnings in `run.log`; point-probe records in `witness_series_00.npz` and `witness_series_01.npz`.','',
              ...
              'Commands: [COMMANDS.md](COMMANDS.md), [COMMANDS_addendum.md](COMMANDS_addendum.md). Measurement source: [measure.py](measure.py). Reduction: [reduce.py](reduce.py). Verification: [verification.json](verification.json).','',
              '**EDGES.md — inline**','',(OUT/'EDGES.md').read_text(),
              '**TABLE.md — inline; Markdown delimiters escaped**','',(OUT/'TABLE_display.md').read_text(),
              '**Files and sizes**','']
    body='\n'.join(lines)
