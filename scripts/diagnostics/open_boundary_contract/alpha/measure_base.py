...
def build_pair(fixture, variant):
    ...
    newlo[0] = -(grid.pad_x_lo + 1) * float(grid.dx)
    newhi[0] = float(baseline._domain[0]) + (grid.pad_x_hi + 1) * float(grid.dx)
    calls = []
    ...
    try:
        ...
        def observe(run_grid, materials, n_steps, *args, **kwargs):
            ...
            if variant == 'continued':
                counts = record['planes'][0]['edge_counts']
                a2 = all(counts[a][f'centre_row_x_{s}'] > 0 for a in 'xy' for s in ('lo','hi'))
                status['A2_edges_present'] = int(a2)
                if not a2:
                    raise RuntimeError('A2: zero trace-edge count in at least one x absorber; ' + json.dumps(counts))
            if dry:
                raise Captured()
...
def main():
    ...
    dest = OUT / (args.fixture + '_dry' if args.dry_run else args.fixture)
    dest.mkdir()
    ...
    variants = ('baseline',) if args.dry_run else ('baseline','continued')
    statuses = []
