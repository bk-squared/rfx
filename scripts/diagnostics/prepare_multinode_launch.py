...
def main():
    ...
    for nx in (200, 400, 800):
        ...
        path = out / f"oracle-{nx}.yaml"
        path.write_text(yaml.dump(template, sort_keys=False))
        oracle_commands.append(command(["sh", root / "scripts/vessl_submit.sh", path,
                                        f"oracle-{nx}", artifacts]) + " &")
        oracle_commands.append(f"p{nx}=$!")
    oracle_commands.extend(["rc=0", "for pid in \"$p200\" \"$p400\" \"$p800\"; do",
    ...
    for slab in (100, 200, 400):
        ...
        cli = ["vessl", "experiment", "create", "--project", "byungkwan", "--cluster", "remilab-c0",
               "--resource", "gpu-rtx4090", "--image-url", "nvcr.io/nvidia/jax:24.10-py3",
        ...
        download = ["#!/bin/sh", "set -eu", command(["cd", out]), f'number=$(cat {number_file})',
                    ...
                    command(["cp", "-R", str(out / f"download-{job}" / f"multinode-{args.stamp}" / job) + "/.", receipt]),
