...
ARTIFACTS.  ``scripts/diagnostics/_artifacts/cv03_seam_facet/<stage>.json``,
the layout ``scripts/diagnostics/_artifacts/cv01_cpml_813/`` established.
``plot_seam_facet.py`` beside this file renders them into
``seam_facet.png`` in the same artifact directory.

Run (2-D, CPU, ~20 s per arm -- no VESSL):

    for s in profile tof sweep noext bprime; do
      PYTHONPATH=<repo> python3 scripts/diagnostics/cv03_seam_facet/seam_facet.py \
        --stage $s \
        --output scripts/diagnostics/_artifacts/cv03_seam_facet/$s.json
    done
"""
...
# BEFORE anything can pull JAX in.  The ``cv01`` stage runs cv01's rig
# IN-PROCESS, and that rig underflows float32: its flux monitors integrate
# E x H* products around 1e-56, so ``flux_spectrum`` returns exactly 0.0 at
# every bin and ``mean_self`` comes back NaN (issue #304).  The committed cv01
# driver sets this on its own first line for the same reason; a driver that
# imports rfx before setting it gets a silently different solver.  The
# subprocess stages inherit it, and the cv03 case sets it for itself anyway.
...
# ---------------------------------------------------------------------------
_DUMP = '''
# --- appended by scripts/diagnostics/cv03_seam_facet/seam_facet.py ---
