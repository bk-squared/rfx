...
def make_geometry():
    ...
    ax.annotate("Patch (PEC)", xy=(L * 0.25 * mm, z_sub_top * mm + 0.18),
                xytext=(L * 0.25 * mm, z_sub_top * mm + 1.05), ha="center",
                va="bottom", fontsize=9,
                arrowprops=dict(arrowstyle="->", color="0.3", lw=0.8))
    ax.annotate(f"FR4  εr = {EPS_R},  h = {H_SUB*1e3:.1f} mm,  tanδ = {TAN_D}",
                xy=(gx * 0.18 * mm, H_SUB * mm * 0.5),
                xytext=(0, -1.9), ha="center", fontsize=8.5,
                arrowprops=dict(arrowstyle="->", color="0.3", lw=0.8))
    ax.annotate("Ground plane (PEC)", xy=(-gx * 0.22 * mm, -0.09),
                xytext=(-gx * 0.18 * mm, -3.05), ha="center", fontsize=9,
                arrowprops=dict(arrowstyle="->", color="0.3", lw=0.8))
    ax.annotate(f"50 Ω wire port (Ez)\ninset {inset*1e3:.1f} mm from edge",
                xy=(fx * mm, H_SUB * mm * 0.5), xytext=(-gx * 0.46 * mm, -2.4),
    ...
    ax.text(-L * 0.2 * mm, y_dim + 0.06, f"L = {L*1e3:.1f} mm", ha="center",
...
def make_field():
    ...
    ax.set_title(f"E$_z$ on the patch plane — TM$_{{010}}$ at {f_res/1e9:.2f} GHz\n"
                 "half-wave standing wave along L, uniform along W", fontsize=10.5)
    ax.set_aspect("equal")
...
def make_s11():
    ...
    # annotate to the LEFT of the dip (the zoom inset sits lower-right)
    ax.annotate(f"matched |S11| dip\n{f_dip:.2f} GHz, {db_dip:.1f} dB",
                xy=(f_dip, db_dip), xytext=(f_dip - 0.55, db_dip + 1.2),
                fontsize=9, color="#b00000", va="center", ha="right",
                arrowprops=dict(arrowstyle="->", color="#b00000", lw=0.9))
    ax.axvline(f_res, color="0.4", ls="--", lw=1.1)
    ax.text(f_res - 0.03, db.max() - 0.4,
