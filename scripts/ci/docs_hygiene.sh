...
# CLAUDE.md, AGENTS.md or .claude/ to point at.
#
# AGENTS.md joined the list on 2026-09-19: it is gitignored like CLAUDE.md, so a
# public clone has no such file, and docs/agent/working-on-rfx.mdx had sent a
# reader to it three times. A review caught that one; this makes it a gate.
#
...
hits=$(grep -rn 'research_notes\|agent-memory\|CLAUDE\.md\|AGENTS\.md\|\.claude/' docs/agent/ || true)
if [ -n "$hits" ]; then
...
hits=$(grep -rn 'agent-memory\|CLAUDE\.md\|AGENTS\.md\|\.claude/' rfx/ examples/ || true)
if [ -n "$hits" ]; then
