...
def box_at_realized_faces(shape, grid, domain):
    """Realize a Box's declared face attachments at absorbing grid faces (C3).

    Non-Box shapes are returned unchanged; no rule for moving their bounds
    is defined here. Declarations are never mutated. A zero-thickness Box
    remains zero-thickness when its plane follows a face.
    """
    from rfx.geometry.csg import Box
    from rfx.grid import CELL_COUNT_ULP_BUDGET

    if not isinstance(shape, Box) or domain is None:
        return shape
    ...
    for axis, nodes in enumerate(coords[:3]):
        pl, ph = grid.face_pads[2 * axis:2 * axis + 2]
        if not (pl or ph):
            continue
        if is_tracer(nodes):
            raise NotImplementedError(
                "C3 face attachment on a traced mesh requires a shape-bound "
                "representation that accepts traced coordinates")
        # cells_spanning compares a dimensionless ratio with
        # budget * eps * max(1, ratio); multiply by the local cell width.
        step = float(nodes[pl + 1] - nodes[pl])
        tol = CELL_COUNT_ULP_BUDGET * np.finfo(float).eps * max(
            step, abs(float(domain[axis])))
        was_plane = lo[axis] == hi[axis]
        if pl and lo[axis] <= tol:
            lo[axis] = float(nodes[pl])
            if was_plane:
                hi[axis] = lo[axis]
        if ph and shape.corner_hi[axis] >= float(domain[axis]) - tol:
            hi[axis] = float(nodes[len(nodes) - ph - 1])
            if was_plane:
                lo[axis] = hi[axis]
    if tuple(lo) == shape.corner_lo and tuple(hi) == shape.corner_hi:
        return shape
    return Box(tuple(lo), tuple(hi))


def extend_cpml_pad_conductors(
    pec_mask,
    sheets,
    wires,
    sheet_specs,
    ...
    """Copy realized conductor layers through absorbing pads (C2).

    Volumes use cell layers, sheet footprints use node planes along their
    in-plane axes, and each wire component uses cells along its own axis
    and nodes along the other two. The terminal storage plane on a cell
    axis is not a physical cell and is left unchanged. Passes run x, y, z,
    lo then hi, as in :func:`extend_cpml_pad_materials`.

    This primitive does not construct port exceptions or assemble a model.
    It is not connected to the assemblers: the shape-based conductor lanes,
    conductivity-folded conductors, and port-window integration are pending
    the open-boundary implementation report's recorded design conflicts.
    No material array or dispersion pole is an input or output.
    """
    from dataclasses import replace

    pads = ((plx, phx), (ply, phy), (plz, phz))

    def extend(array, cell_axes=(), normal_axis=None):
        out = array
        valid = [slice(0, n - int(axis in cell_axes))
                 for axis, n in enumerate(array.shape)]
        for axis, (lo, hi) in enumerate(pads):
            if axis == normal_axis:
                continue
            end = array.shape[axis] - int(axis in cell_axes)
            for depth, source, destination in (
                (lo, lo, slice(0, lo)),
                (hi, end - hi - 1, slice(end - hi, end)),
            ):
                if depth <= 0:
                    continue
                src, dst = list(valid), list(valid)
                src[axis] = slice(source, source + 1)
                dst[axis] = destination
                out = out.at[tuple(dst)].set(out[tuple(src)])
        return out

    cells = (None if pec_mask is None
             else extend(pec_mask, cell_axes=(0, 1, 2)))
    continued_sheets = tuple(
        replace(sheet, footprint=extend(
            sheet.footprint, normal_axis=sheet.normal_axis))
        for sheet in sheets)
    continued_wires = tuple(
        replace(wire, edges=tuple(extend(edge, cell_axes=(component,))
                                 for component, edge in enumerate(wire.edges)))
        for wire in wires)
    continued_impedance = tuple(
        replace(sheet,
                mask=extend(sheet.mask, normal_axis=sheet.normal_axis),
                sigma_sheet=extend(sheet.sigma_sheet,
                                   normal_axis=sheet.normal_axis))
        for sheet in sheet_specs)
    return cells, continued_sheets, continued_wires, continued_impedance
