...
and left in it; a lock in ``tests/locks/`` pins the 37 names, in order,
against a literal tuple derived by an AST walk of the PRE-registry hub body.
...
# The core sequence.
#
# DERIVATION, so this is reviewable rather than trusted: the 37 entries below
...
CORE_CONFIG_CHECKS: tuple[ConfigCheck, ...] = (
    ...
    ConfigCheck("_validate_cfg_ntff_absorber_overlap",
                lambda sim, c: sim._validate_cfg_ntff_absorber_overlap(
                    ...
                    c.absorber_label),
                "ntff"),
    ConfigCheck("_validate_cfg_ntff_min_steps",
                lambda sim, c: sim._validate_cfg_ntff_min_steps(c.dx),
                "ntff"),
